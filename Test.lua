...
local TweenService = game:GetService("TweenService") -- [TAMBAH] TweenService
...
-- Gap Up & Down Buttons (ATAS Fly to Base)
MainTab:Button({
    Title = "Gap Up",
    Desc = "Select next gap (" .. #GapPositions .. " Total)",
    Callback = function()
        CurrentGap = CurrentGap + 1
        if CurrentGap > #GapPositions then
            CurrentGap = 1
        end
        WindUI:Notify({
            Title = "Gap Selected",
            Content = "Gap " .. CurrentGap,
            Duration = 1,
            Icon = "arrow-up"
        })
    end
...
-- Anti Tsunami & Gap Tween (BAWAH Fly to Base)

-- Tween to selected Gap Dropdown
...
local SpeedToggle = MainTab:Toggle({
    ...
    Callback = function(state)
        ...
        States.Speed = state -- Sync with Anti Tsunami
    ...
    autoBuySpeedConnection = RunService.Heartbeat:Connect(function()
        if isAutoBuySpeedEnabled then
            ...
            if currentTime - lastUpgradeTime >= upgradeDelay then
                local amount = 1
                if selectedSpeedOption == "+5" then amount = 5
                elseif selectedSpeedOption == "+10" then amount = 10 end
...
local function startAutoUpgradeBase()
    ...
    baseThread = task.spawn(function()
        while autoUpgradeBaseEnabled do
            pcall(function()
                ReplicatedStorage:WaitForChild("Packages"):WaitForChild("Net"):WaitForChild("RE/Plot.UpgradeBase"):FireServer()
            end)
...
local function toggleESPCelestial(state)
    ...
    if state then
        ...
        if ConnectionsESP.ESPCelestial then
            ConnectionsESP.ESPCelestial:Disconnect()
            ConnectionsESP.ESPCelestial = nil
        end
        clearESPCelestial()
    end
end
...
local function toggleESPLuckyBlock(state)
    ...
    if state then
        ...
        if ConnectionsESP.ESPLuckyBlock then
            ConnectionsESP.ESPLuckyBlock:Disconnect()
            ConnectionsESP.ESPLuckyBlock = nil
        end
        clearESPLuckyBlock()
    end
end
...
myConfig:Register("AntiTsunami", MainTab:GetElement("Anti Tsunami")) -- Reference to toggle
...
if GapDropdown and GapDropdown.Value then
    CurrentGap = tonumber(GapDropdown.Value:match("%d+")) or 1
end
...
LocalPlayer.CharacterAdded:Connect(function(character)
    ...
    -- Call the Anti Tsunami respawn handler
