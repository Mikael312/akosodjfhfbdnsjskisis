...
local function setupDrag(frame, posKey)
    ...
    -- Fallback: lepas tetikus keluar frame pun kira release
...
-- Lock Gui — save & load betul, update guiLocked flag
makeIosToggle("Lock Gui", credScroll, 6, function(state)
    guiLocked = state
    -- Config disimpan dalam makeIosToggle secara auto
end)
...
makeIosToggle("Save Gui Position", credScroll, 7, function(state)
    ...
    -- Config toggle disimpan dalam makeIosToggle secara auto
end)
...
task.defer(function()
    ...
    if pos then
        if pos.main   then mainFrame.Position   = UDim2.new(0, pos.main.x,   0, pos.main.y)   end
        if pos.menu   then menuFrame.Position   = UDim2.new(0, pos.menu.x,   0, pos.menu.y)   end
        if pos.credit then creditFrame.Position = UDim2.new(0, pos.credit.x, 0, pos.credit.y) end
    end

    -- Setup drag selepas positions loaded
