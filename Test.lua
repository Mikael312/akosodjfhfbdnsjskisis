...
-- ==================== LOAD CONFIG ====================
local config = ConfigSystem:Load()

-- ==================== ESP PLAYERS VARIABLES ====================
local espPlayersEnabled = config.espPlayers or false
local espObjects = {}
...
local espBestEnabled = config.espBest or false
...
-- ==================== BASE LINE VARIABLES ====================
local baseLineEnabled = config.baseLine or false
...
local sentryEnabled = config.antiTurret or false
...
-- ==================== AIMBOT VARIABLES ====================
local autoLaserEnabled = config.aimbot or false
...
local isMonitoring = config.kickSteal or false
local lastStealCount = 0
local monitoringLoop = nil

-- ==================== UNWALK ANIM VARIABLES ====================
local unwalkAnimEnabled = config.unwalkAnim or false
local unwalkAnimConnections = {}

-- ==================== AUTO STEAL VARIABLES ====================
local autoStealEnabled = config.autoSteal or false
...
local antiRagdollActive = config.antiRagdoll or false
local ragdollConnections = {}
local cachedCharData = {}

-- ==================== XRAY BASE VARIABLES ====================
local xrayBaseEnabled = config.xrayBase or false
local invisibleWallsLoaded = false
local originalTransparency = {}
local xrayBaseConnection = nil

-- ==================== FPS BOOST VARIABLES ====================
local fpsBoostEnabled = config.fpsBoost or false
local optimizerThreads = {}
local optimizerConnections = {}
local originalSettings = {}

-- ==================== ESP TIMER VARIABLES ====================
local timerEspEnabled = config.espTimer or false
...
local function enableESPPlayers()
    ...
    ConfigSystem:UpdateSetting(config, "espPlayers", true)
...
local function disableESPPlayers()
    ...
    ConfigSystem:UpdateSetting(config, "espPlayers", false)
...
local function enableESPBest()
    ...
    espBestEnabled = true
    ConfigSystem:UpdateSetting(config, "espBest", true)
...
local function disableESPBest()
    ...
    ConfigSystem:UpdateSetting(config, "espBest", false)
...
local function enableBaseLine()
    ...
    ConfigSystem:UpdateSetting(config, "baseLine", true)
...
local function disableBaseLine()
    ...
    ConfigSystem:UpdateSetting(config, "baseLine", false)
    pcall(stopPlotLine)
...
local function enableAntiTurret()
    ...
    sentryEnabled = true
    ConfigSystem:UpdateSetting(config, "antiTurret", true)
    startSentryWatch()
...
local function disableAntiTurret()
    if not sentryEnabled then return end
    sentryEnabled = false
    ConfigSystem:UpdateSetting(config, "antiTurret", false)
    stopSentryWatch()
...
local function enableAimbot()
    ...
    ConfigSystem:UpdateSetting(config, "aimbot", true)
...
local function disableAimbot()
    ...
    ConfigSystem:UpdateSetting(config, "aimbot", false)
...
local function enableKickSteal()
    if isMonitoring then return end
    ConfigSystem:UpdateSetting(config, "kickSteal", true)
    startMonitoring()
...
local function disableKickSteal()
    if not isMonitoring then return end
    ConfigSystem:UpdateSetting(config, "kickSteal", false)
    stopMonitoring()
...
local function enableUnwalkAnim()
    ...
    ConfigSystem:UpdateSetting(config, "unwalkAnim", true)
...
local function disableUnwalkAnim()
    ...
    ConfigSystem:UpdateSetting(config, "unwalkAnim", false)
...
local function enableAutoSteal()
    ...
    ConfigSystem:UpdateSetting(config, "autoSteal", true)
...
local function disableAutoSteal()
    ...
    ConfigSystem:UpdateSetting(config, "autoSteal", false)
...
local function toggleAntiDebuff(state)
    ...
    ConfigSystem:UpdateSetting(config, "antiDebuff", state)
    if state then
...
function ANTI_RAGDOLL.Enable()
    ...
    ConfigSystem:UpdateSetting(config, "antiRagdoll", true)
...
function ANTI_RAGDOLL.Disable()
    ...
    ConfigSystem:UpdateSetting(config, "antiRagdoll", false)
...
local function enableXrayBase()
    ...
    ConfigSystem:UpdateSetting(config, "xrayBase", true)
...
local function disableXrayBase()
    ...
    ConfigSystem:UpdateSetting(config, "xrayBase", false)
    invisibleWallsLoaded = false
...
local function enableFpsBoost()
    ...
    ConfigSystem:UpdateSetting(config, "fpsBoost", true)
...
local function disableFpsBoost()
    ...
    ConfigSystem:UpdateSetting(config, "fpsBoost", false)
...
local function enableTimerESP()
    ...
    ConfigSystem:UpdateSetting(config, "espTimer", true)
...
local function disableTimerESP()
    ...
    ConfigSystem:UpdateSetting(config, "espTimer", false)
...
for _, gui in pairs(game.CoreGui:GetChildren()) do
    if gui.Name == "NightmareUI" then
        gui:Destroy()
    end
end

local screenGui = Instance.new("ScreenGui")
screenGui.Name = "NightmareUI"
screenGui.ResetOnSpawn = false
screenGui.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
screenGui.Parent = game.CoreGui

-- ==================== TOGGLE BUTTON (Dragable) ====================
...
-- Main Frame (Lebih lebar & panjang sedikit + ruang untuk button)
...
-- ScrollingFrame untuk toggle (boleh scroll kalau banyak)
...
scrollFrame.CanvasSize = UDim2.new(0, 0, 0, 60)
scrollFrame.Parent = mainFrame

-- UIListLayout untuk auto-arrange toggles
...
-- ==================== SOCIAL BUTTONS (Fixed, Tak Scroll) ====================
...
ToggleButton.MouseButton1Click:Connect(function()
    ...
    mainFrame.Visible = isUIVisible -- No animation, instant show/hide
...
local function createToggleRow(text1, text2, callback1, callback2, initialState1, initialState2)
    ...
    toggle1.Size = UDim2.new(0, 100, 0, 32)
    toggle1.Position = UDim2.new(0, 5, 0, 0)
    toggle1.BackgroundColor3 = initialState1 and Color3.fromRGB(200, 30, 30) or Color3.fromRGB(80, 0, 0)
    toggle1.BorderSizePixel = 0
    toggle1.Text = text1
    toggle1.TextColor3 = Color3.fromRGB(255, 255, 255)
    toggle1.TextSize = 13
    toggle1.Font = Enum.Font.Arcade
    toggle1.Parent = rowFrame
    ...
    stroke1.Thickness = 1
    stroke1.Parent = toggle1
    
    local isToggled1 = initialState1 or false
    toggle1.MouseButton1Click:Connect(function()
        isToggled1 = not isToggled1
        if isToggled1 then
            toggle1.BackgroundColor3 = Color3.fromRGB(200, 30, 30)
            print("🔴 " .. text1 .. ": ON")
        else
            toggle1.BackgroundColor3 = Color3.fromRGB(80, 0, 0)
            print("⚫ " .. text1 .. ": OFF")
        end
        
        if callback1 then
            callback1(isToggled1)
        end
    end)
    ...
    if text2 and text2 ~= "" then
        ...
        toggle2.Size = UDim2.new(0, 100, 0, 32)
        toggle2.Position = UDim2.new(0, 115, 0, 0)
        toggle2.BackgroundColor3 = initialState2 and Color3.fromRGB(200, 30, 30) or Color3.fromRGB(80, 0, 0)
        toggle2.BorderSizePixel = 0
        toggle2.Text = text2
        toggle2.TextColor3 = Color3.fromRGB(255, 255, 255)
        toggle2.TextSize = 13
        toggle2.Font = Enum.Font.Arcade
        toggle2.Parent = rowFrame
        ...
        stroke2.Thickness = 1
        stroke2.Parent = toggle2
        
        local isToggled2 = initialState2 or false
        toggle2.MouseButton1Click:Connect(function()
            isToggled2 = not isToggled2
            if isToggled2 then
                toggle2.BackgroundColor3 = Color3.fromRGB(200, 30, 30)
                print("🔴 " .. text2 .. ": ON")
            else
                toggle2.BackgroundColor3 = Color3.fromRGB(80, 0, 0)
                print("⚫ " .. text2 .. ": OFF")
            end
            
            if callback2 then
                callback2(isToggled2)
            end
        end)
    end
...
-- ==================== CREATE TOGGLES ====================
createToggleRow("Esp Players", "Esp Best", toggleEspPlayers, toggleEspBest, espPlayersEnabled, espBestEnabled)
createToggleRow("Base Line", "Anti Turret", toggleBaseLine, toggleAntiTurret, baseLineEnabled, sentryEnabled)
createToggleRow("Aimbot", "Kick Steal", toggleAimbot, toggleKickSteal, autoLaserEnabled, isMonitoring)
createToggleRow("Unwalk Anim", "Auto Steal", toggleUnwalkAnim, toggleAutoSteal, unwalkAnimEnabled, autoStealEnabled)
createToggleRow("Anti Debuff", "Anti Rdoll", toggleAntiDebuff, toggleAntiRagdoll, false, antiRagdollActive)
createToggleRow("Xray Base", "Fps Boost", toggleXrayBase, toggleFpsBoost, xrayBaseEnabled, fpsBoostEnabled)
createToggleRow("Esp Timer", "", toggleEspTimer, nil, timerEspEnabled, false)
...
-- ==================== INITIALIZATION ====================
print("==========================================")
print("🎮 NIGHTMARE UI LOADED!")
print("==========================================")
print("📐 Size: 240x380")
print("🔘 Toggles: Esp Players, Esp Best, Base Line, Anti Turret, Aimbot, Kick Steal, Unwalk Anim, Auto Steal, Anti Debuff, Anti Rdoll, Xray Base, Fps Boost, Esp Timer")
print("📜 Scrollable: YES")
print("🎮 Toggle Button: Dragable (Click to show/hide)")
print("🔗 Social: TikTok & Discord")
print("📋 Click buttons to copy links!")
print("==========================================")

-- Auto-enable features from config
if espPlayersEnabled then enableESPPlayers() end
if espBestEnabled then enableESPBest() end
if baseLineEnabled then enableBaseLine() end
if sentryEnabled then enableAntiTurret() end
if autoLaserEnabled then enableAimbot() end
if isMonitoring then enableKickSteal() end
if unwalkAnimEnabled then enableUnwalkAnim() end
if autoStealEnabled then enableAutoSteal() end
if antiRagdollActive then ANTI_RAGDOLL.Enable() end
if xrayBaseEnabled then enableXrayBase() end
if fpsBoostEnabled then enableFpsBoost() end
if timerEspEnabled then enableTimerESP() end
