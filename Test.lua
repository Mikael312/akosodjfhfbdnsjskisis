...
local brainrotSlotsCount = 0 -- Track the number of slots
...
-- ==================== UPDATE BRAINROT SLOTS DROPDOWN ====================
local function updateBrainrotSlotsDropdown()
    local slots = getBrainrotSlots()
    brainrotSlotsCount = #slots
    ...
    for i = 1, brainrotSlotsCount do
        table.insert(dropdownValues, {Title = "Slot " .. i, Icon = "numeric-" .. i})
    end
    
    -- Update dropdown if it exists
    if BrainrotUpgradeModeDropdown then
        BrainrotUpgradeModeDropdown:Set({
...
-- Brainrot Upgrade Mode Dropdown
local BrainrotUpgradeModeDropdown = AutoTab:Dropdown({
...
local function createESP(player)
    ...
    esp.highlight.FillTransparency = 0.5 -- Make highlight always visible
        ...
        if player.Character and player.Character:FindFirstChild("HumanoidRootPart") and player.Character:FindFirstChild("Humanoid") then
            ...
            if humanoid.Health > 0 then
                ...
                if espEnabled then
                    ...
                    esp.highlight.Enabled = true -- Always enable highlight regardless of distance
                    ...
                    if onScreen then
                        ...
                        esp.box.Visible = false
                        esp.name.Visible = false
                        -- Keep highlight visible even when off-screen
                    end
                else
                    esp.highlight.Enabled = false
                    esp.box.Visible = false
...
local function createESPCelestial(part, text)
    ...
    textLabel.Font = Enum.Font.Gotham -- Changed from GothamBold to Gotham
            ...
            pcall(function()
                ...
                for _, block in pairs(activeLuckyBlocks:GetChildren()) do
                    if block:IsA("Model") and string.find(block.Name, "NaturalLuckyBlock") then
                        ...
                        if primary and not primary:FindFirstChild("ESP_LuckyBlock_") then
                            createESPLuckyBlock(primary, "Lucky Block")
                        end
                    end
                end
            end)
...
local EspLuckyBlockToggle = VisualTab:Toggle({
    ...
    Desc = "Highlight all Lucky Blocks",
    Default = false,
    Callback = function(state)
...
-- UPDATE BRAINROT SLOTS DROPDOWN
-- ========================================
-- Function to update the brainrot slots dropdown
local function updateBrainrotSlotsDropdown()
    local slots = getBrainrotSlots()
    brainrotSlotsCount = #slots
...
-- Check for brainrot slots every 5 seconds
task.spawn(function()
    while true do
        task.wait(5)
        local currentSlotsCount = #getBrainrotSlots()
        if currentSlotsCount ~= brainrotSlotsCount then
            updateBrainrotSlotsDropdown()
        end
    end
end)
