...
-- Auto-equip Grapple Hook (GUNA YANG LEBIH ROBUST)
...
-- Fire Grapple Hook (GUNA YANG LEBIH ROBUST)
...
local function startVehicleFly()
    ...
    if not root then
        warn("[Vehicle Fly] No root part found!")
        return
    end
    ...
    mfly2 = RunService.RenderStepped:Connect(function()
        ...
        if player.Character:FindFirstChildWhichIsA("Humanoid") and root and root:FindFirstChild(velocityHandlerName) and root:FindFirstChild(gyroHandlerName) then
            ...
            if direction.X > 0 then
                VelocityHandler.Velocity = VelocityHandler.Velocity + camera.CFrame.RightVector * (direction.X * (vehicleflyspeed * 50))
            end
            if direction.X < 0 then
                VelocityHandler.Velocity = VelocityHandler.Velocity + camera.CFrame.RightVector * (direction.X * (vehicleflyspeed * 50))
            end
            if direction.Z > 0 then
                VelocityHandler.Velocity = VelocityHandler.Velocity - camera.CFrame.LookVector * (direction.Z * (vehicleflyspeed * 50))
            end
            if direction.Z < 0 then
                VelocityHandler.Velocity = VelocityHandler.Velocity - camera.CFrame.LookVector * (direction.Z * (vehicleflyspeed * 50))
            end
            ...
            if UserInputService:IsKeyDown(Enum.KeyCode.E) then
                VelocityHandler.Velocity = VelocityHandler.Velocity + Vector3.new(0, vehicleflyspeed * 50, 0)
            end
    ...
    stealCheckConnection = RunService.Heartbeat:Connect(function()
        ...
        if isStealingNow == true then
            print("[🛑] Stealing detected! Auto OFF fly...")
            NOFLY()
        end
    end)
end
...
mainFrame.Size = UDim2.new(0, 180, 0, 320) -- DITINGGIKAN UNTUK BUTANG BAHARU
mainFrame.Position = UDim2.new(1, -290, 0.5, -160) -- DISESUAIKAN UNTUK CENTERING BARU
...
-- ==================== TOGGLE BUTTON FLY V1 (NEW) ====================
local isFlyV1Toggled = false
local toggleButtonFlyV1 = createToggleButton(mainFrame, "FlyV1", "Fly V1", UDim2.new(0.5, -80, 0, 220), UDim2.new(0, 160, 0, 32))

toggleButtonFlyV1.MouseButton1Click:Connect(function()
    isFlyV1Toggled = not isFlyV1Toggled
    setToggleState(toggleButtonFlyV1, isFlyV1Toggled)
    
    if isFlyV1Toggled then
        startVehicleFly()
    else
        NOFLY()
    end
...
local toggleButton5 = createToggleButton(mainFrame, "StealFloor", "Steal Floor", UDim2.new(0.5, -80, 0, 260), UDim2.new(0, 160, 0, 32))
...
contentLabel.Position = UDim2.new(0, 20, 0, 290) -- DISESUAIKAN
...
LocalPlayer.CharacterAdded:Connect(function()
    ...
    -- Reset Fly V1 on respawn
    if isFlyV1Toggled then
        isFlyV1Toggled = false
        setToggleState(toggleButtonFlyV1, isFlyV1Toggled)
        NOFLY()
    end
