...
-- ========================================
-- GLOBAL STATES & CONNECTIONS (MERGED)
-- ========================================
local States = {
    DebugMode = false,
    ESPCelestial = false,
    ...
    Speed = false, -- Akan di-sync dengan Speed Toggle
    JumpPower = false, -- Akan di-sync dengan JumpBoost Toggle
    CharacterAlive = true
}
...
-- ========================================
-- ANTI TSUNAMI & GAP LOGIC FUNCTIONS
-- ========================================
            ...
            if child:IsA("Model") and child.Name:match("Wave") then
                ...
                if hitbox and hitbox:IsA("BasePart") then
                    ...
                    for _, existingWave in ipairs(WaveCache.Waves) do
                        if existingWave.Wave == child then alreadyAdded = true break end
                    end
...
local function isWaveBlockingGap(playerXPos, gapXPos, waves)
    for _, wave in ipairs(waves) do
        if playerXPos < gapXPos then
            if wave.XPosition > playerXPos and wave.XPosition < gapXPos then return true end
        else
            if wave.XPosition < playerXPos and wave.XPosition > gapXPos then return true end
        end
    end
...
local function findBestGapToRetreat(playerPosition, wavePosition, gaps, allWaves)
    ...
    for _, gap in ipairs(gaps) do
        if gap.XPosition > playerX then table.insert(forwardGaps, gap)
        else table.insert(backwardGaps, gap) end
    end
    
    if #forwardGaps > 0 then
        table.sort(forwardGaps, function(a, b) return a.XPosition < b.XPosition end)
        ...
        if not isBlocked then
            ...
            for _, wave in ipairs(allWaves) do
                ...
                if distToGap < 30 then isSafe = false break end
                if wave.XPosition > playerX and wave.XPosition < nearestForward.XPosition then isSafe = false break end
                if wave.XPosition < playerX and math.abs(wave.XPosition - playerX) < 40 then isSafe = false break end
                if wave.XPosition > nearestForward.XPosition and wave.XPosition < nearestForward.XPosition + 40 then isSafe = false break end
            end
            if isSafe then return nearestForward, true end
        end
    end
    
    if #backwardGaps > 0 then
        table.sort(backwardGaps, function(a, b) return b.XPosition < a.XPosition end)
        local nearestBackward = backwardGaps[1]
        if not isWaveBlockingGap(playerX, nearestBackward.XPosition, allWaves) then return nearestBackward, false end
    end
    ...
    for _, gap in ipairs(gaps) do
        ...
        if dist > farthestDist then farthestDist = dist farthestGap = gap end
    end
...
local function tweenToGap(hrp, targetGap, isForward)
    ...
    if not targetGap then return end -- [FIX] Prevent crash if no gap found
    ...
    if horizontalDist <= 50 then
        ...
        task.wait(0.1)
        if humanoid and not States.Speed then humanoid.WalkSpeed = 16 end
        if humanoid and not States.JumpPower then humanoid.JumpPower = 50 end
...
function toggleAntiTsunami(state)
    ...
    if state then
        WindUI:Notify({Title = "Anti Tsunami", Content = "Anti Tsunami ENABLED", Duration = 2, Icon = "shield"})
        ...
        Connections.AntiTsunami = RunService.Heartbeat:Connect(function()
            ...
            pcall(function()
                ...
                if nearestWave then
                    if playerPosition.X < nearestWave.XPosition then waveIsBehind = false
                    else if (playerPosition.X - nearestWave.XPosition) > 10 then waveIsBehind = true end end
                end

                if waveIsBehind then return end
                
                if (playerPosition.Y >= -4 and playerPosition.Y <= 2) or playerPosition.X < 152 then return end
                ...
                for _, gap in ipairs(gaps) do
                    ...
                    if distToGap < currentGapSafetyMargin and playerPosition.Y < 0 then playerInSafeGap = true break end
                end
                ...
                if playerInSafeGap then
                    if nearestWave.Distance > 60 and distToBestGap < 100 then return end
                    if nearestWave.Distance <= 60 then -- Continue to evacuation
                    elseif distToBestGap >= 100 then -- Continue to evacuation
                    else return end
                end
                
                if distToBestGap < 10 and playerPosition.Y < -1 then return end
                ...
                if isWaveBlockingGap(playerPosition.X, bestGap.XPosition, allWaves) then
                    ...
                    for _, gap in ipairs(gaps) do
                        ...
                        if gapDirection ~= targetDirection then
                            if not isWaveBlockingGap(playerPosition.X, gap.XPosition, allWaves) then table.insert(alternativeGaps, gap) end
                        end
                    end
                    if #alternativeGaps > 0 then
                        table.sort(alternativeGaps, function(a, b) return math.abs(playerPosition.X - a.XPosition) < math.abs(playerPosition.X - b.XPosition) end)
        ...
        WindUI:Notify({Title = "Anti Tsunami", Content = "Anti Tsunami DISABLED", Duration = 2, Icon = "shield-off"})
        if Connections.AntiTsunami then Connections.AntiTsunami:Disconnect() Connections.AntiTsunami = nil end
        if States.CurrentTween then States.CurrentTween:Cancel() States.CurrentTween = nil end
        States.MovementDisabled = false
...
-- ========================================
-- MAIN TAB ELEMENTS (UPDATED)
-- ========================================

-- Gap Up & Down Buttons
MainTab:Button({
    Title = "Gap Up",
    Desc = "Select next gap (" .. #GapPositions .. " Total)",
    Callback = function()
        CurrentGap = CurrentGap + 1
        if CurrentGap > #GapPositions then CurrentGap = 1 end
        WindUI:Notify({Title = "Gap Selected", Content = "Gap " .. CurrentGap, Duration = 1, Icon = "arrow-up"})
    end
})

MainTab:Button({
    Title = "Gap Down",
    Desc = "Select previous gap",
    Callback = function()
        CurrentGap = CurrentGap - 1
        if CurrentGap < 1 then CurrentGap = #GapPositions end
        WindUI:Notify({Title = "Gap Selected", Content = "Gap " .. CurrentGap, Duration = 1, Icon = "arrow-down"})
    end
})
...
local function cleanupFlyToBase()
    ...
    if currentVelocity then pcall(function() currentVelocity:Destroy() end) currentVelocity = nil end
    if currentAttachment then pcall(function() currentAttachment:Destroy() end) currentAttachment = nil end
    isTeleporting = false
end
...
local function getMyBase()
    for _, base in pairs(Bases:GetChildren()) do
        ...
        if holder and holder == LocalPlayer.UserId then return base end
    end
    return nil
end
...
local function flyToBase()
    ...
    heartbeatConnection = RunService.Heartbeat:Connect(function()
        if not char or not char.Parent or not hrp or not hrp.Parent or not hum or hum.Health <= 0 then cleanupFlyToBase() return end
        ...
        if currentVelocity and currentVelocity.Parent then currentVelocity.VectorVelocity = direction * speed
        else cleanupFlyToBase() end
    end)
end
...
local FlyToBaseButton = MainTab:Button({
    ...
    Callback = function() flyToBase() end
})

-- Anti Tsunami & Gap Tween (Below Fly to Base)

local GapDropdown = MainTab:Dropdown({
    Title = "Select Gap to Tween",
    Values = {
        {Title = "Gap 1", Icon = "1"}, {Title = "Gap 2", Icon = "2"}, {Title = "Gap 3", Icon = "3"},
        {Title = "Gap 4", Icon = "4"}, {Title = "Gap 5", Icon = "5"}, {Title = "Gap 6", Icon = "6"},
        {Title = "Gap 7", Icon = "7"}, {Title = "Gap 8", Icon = "8"}, {Title = "Gap 9", Icon = "9"}
    },
    Value = "Gap 1",
    Callback = function(option)
        CurrentGap = tonumber(option.Title:match("%d+")) or 1
        saveConfiguration()
    end
})

MainTab:Button({
    Title = "Tween to Selected Gap",
    Desc = "Teleport you to the selected gap number",
    Callback = function() TweenToGapWithLanding(CurrentGap) end
})

local AntiTsunamiToggle = MainTab:Toggle({
    Title = "Anti Tsunami",
    Desc = "Automatically run to the nearest safe gap when a wave comes",
    Default = false,
...
local SpeedInput = MainTab:Input({
    ...
    Callback = function(input) speedValue = tonumber(input) or 50 saveConfiguration() end
})
...
local SpeedToggle = MainTab:Toggle({
    ...
    Callback = function(state)
        ...
        States.Speed = state -- [FIX] Sync with Anti Tsunami logic
        if state then enableSpeed() else cleanupSpeed() end
        saveConfiguration()
    end
})
...
local GodModeToggle = MainTab:Toggle({
    ...
    Callback = function(state)
        ...
        if state then enableGodMode() else cleanupGodMode() end
        saveConfiguration()
    end
})
...
local RarityDropdown = MainTab:Dropdown({
    Title = "Select Rarity",
    Values = {
        {Title = "Low Tier", Icon = "circle"}, {Title = "Mid Tier", Icon = "diamond"}, {Title = "High Tier", Icon = "crown"},
        {Title = "Common", Icon = "circle"}, {Title = "Uncommon", Icon = "circle"}, {Title = "Rare", Icon = "circle"},
        {Title = "Epic", Icon = "circle"}, {Title = "Legendary", Icon = "star"}, {Title = "Mythical", Icon = "star"},
        {Title = "Cosmic", Icon = "star"}, {Title = "Secret", Icon = "star"}, {Title = "Celestial", Icon = "star"},
    },
    Value = "Low Tier",
    Callback = function(option)
...
local function enableAutoSteal()
    ...
    autoStealConnection = RunService.Heartbeat:Connect(function()
        ...
        if currentTime - lastCacheUpdate >= CACHE_UPDATE_INTERVAL then updateBrainrotCache() lastCacheUpdate = currentTime end
        for i = #brainrotCache, 1, -1 do
            ...
            if data.prompt and data.prompt.Parent and data.prompt.Enabled then
                ...
                if distance <= data.maxDist then task.spawn(function() pcall(fireproximityprompt, data.prompt) end) end
            else table.remove(brainrotCache, i) end
        end
    end)
end
...
local AutoStealToggle = MainTab:Toggle({
    ...
    Callback = function(state)
        ...
        if state then enableAutoSteal() else cleanupAutoSteal() end
        saveConfiguration()
    end
})
        ...
        if isAutoSellEnabled then
            ...
            if currentTime - lastSellTime >= (sellDelayMs / 1000) then
                pcall(function()
                    ...
                    if selectedSellOption == "Sell All" and sellAllRemote then sellAllRemote:InvokeServer()
                    elseif selectedSellOption == "Sell 1 Brainrot" and sellToolRemote then sellToolRemote:InvokeServer() end
...
local AutoSellToggle = AutoTab:Toggle({
    ...
    Callback = function(state)
        ...
        if state then enableAutoSell() else cleanupAutoSell() end
        saveConfiguration()
    end
})
...
local SellOptionDropdown = AutoTab:Dropdown({
    Title = "Sell Option",
    Values = {{Title = "Sell All", Icon = "coins"}, {Title = "Sell 1 Brainrot", Icon = "brain"}},
    Value = "Sell All",
    Callback = function(option) selectedSellOption = option.Title saveConfiguration() end
})
...
local AutoSellDelaySlider = AutoTab:Slider({
    ...
    Callback = function(value) sellDelayMs = value saveConfiguration() end
})
...
local function collectAllMoney()
    ...
    local maxPlots = 0
    if plotFolder then maxPlots = #plotFolder:GetChildren()
    else
        for _, child in pairs(myBase:GetDescendants()) do
            local plotNum = tonumber(child.Name)
            if plotNum and plotNum > maxPlots then maxPlots = plotNum end
        end
        if maxPlots == 0 then maxPlots = 200 end
    end
    local baseId = myBase.Name
    for plotNum = 1, maxPlots do
        local args = {"Collect Money", baseId, tostring(plotNum)}
        task.spawn(function()
            pcall(function() ReplicatedStorage:WaitForChild("Packages"):WaitForChild("Net"):WaitForChild("RF/Plot.PlotAction"):InvokeServer(unpack(args)) end)
        end)
...
local function enableAutoCollect()
    ...
    autoCollectTask = task.spawn(function()
        while isAutoCollectEnabled do collectAllMoney() task.wait(collectDelay) end
    end)
end
...
local AutoCollectToggle = AutoTab:Toggle({
    ...
    Callback = function(state)
        ...
        if state then enableAutoCollect() else cleanupAutoCollect() end
        saveConfiguration()
    end
})
...
local AutoCollectDelaySlider = AutoTab:Slider({
    ...
    Callback = function(value)
        collectDelay = value
        if isAutoCollectEnabled then cleanupAutoCollect() enableAutoCollect() end
        saveConfiguration()
    end
})
...
local function upgradeSpeed(amount)
    local args = {amount}
    pcall(function() ReplicatedStorage:WaitForChild("RemoteFunctions"):WaitForChild("UpgradeSpeed"):InvokeServer(unpack(args)) end)
end

local function cleanupAutoBuySpeed()
    if autoBuySpeedConnection then autoBuySpeedConnection:Disconnect() autoBuySpeedConnection = nil end
end
    ...
    autoBuySpeedConnection = RunService.Heartbeat:Connect(function()
        if isAutoBuySpeedEnabled then
            ...
            if currentTime - lastUpgradeTime >= upgradeDelay then
                local amount = 1
                if selectedSpeedOption == "+5" then amount = 5 elseif selectedSpeedOption == "+10" then amount = 10 end
                upgradeSpeed(amount)
...
local AutoBuySpeedToggle = AutoTab:Toggle({
    ...
    Callback = function(state)
        ...
        if state then enableAutoBuySpeed() else cleanupAutoBuySpeed() end
        saveConfiguration()
    end
})
...
local SpeedOptionDropdown = AutoTab:Dropdown({
    ...
    Values = {{Title = "+1", Icon = "plus"}, {Title = "+5", Icon = "plus"}, {Title = "+10", Icon = "plus"}},
    Value = "+1",
    Callback = function(option) selectedSpeedOption = option.Title saveConfiguration() end
})

local AutoUpgradeDelaySlider = AutoTab:Slider({
    Title = "Auto Upgrade Delay",
    Desc = "Adjust the delay between speed upgrades (seconds)",
    Value = {Min = 0.1, Max = 100, Default = 1},
    Callback = function(value) upgradeDelay = value saveConfiguration() end
})
...
local function startAutoUpgradeCarry()
    ...
    carryThread = task.spawn(function()
        while autoUpgradeCarryEnabled do
            pcall(function() ReplicatedStorage:WaitForChild("RemoteFunctions"):WaitForChild("UpgradeCarry"):InvokeServer() end)
...
local AutoUpgradeCarryToggle = AutoTab:Toggle({
    ...
    Callback = function(state)
        ...
        if state then startAutoUpgradeCarry() else stopAutoUpgradeCarry() end
        saveConfiguration()
    end
})
...
local CarryUpgradeDelaySlider = AutoTab:Slider({
    ...
    Callback = function(value)
        ...
        if autoUpgradeCarryEnabled then stopAutoUpgradeCarry() startAutoUpgradeCarry() end
        saveConfiguration()
    end
})
...
local function startAutoUpgradeBase()
    ...
    baseThread = task.spawn(function()
        while autoUpgradeBaseEnabled do
            pcall(function() ReplicatedStorage:WaitForChild("Packages"):WaitForChild("Net"):WaitForChild("RE/Plot.UpgradeBase"):FireServer() end)
...
local function stopAutoUpgradeBase()
    if baseThread then task.cancel(baseThread) baseThread = nil end
end
...
local AutoUpgradeBaseToggle = AutoTab:Toggle({
    ...
    Callback = function(state)
        ...
        if state then startAutoUpgradeBase() else stopAutoUpgradeBase() end
        saveConfiguration()
    end
})
...
local BaseUpgradeDelaySlider = AutoTab:Slider({
    ...
    Callback = function(value)
        ...
        if autoUpgradeBaseEnabled then stopAutoUpgradeBase() startAutoUpgradeBase() end
        saveConfiguration()
    end
})
...
local function getSlotsToUpgrade()
    ...
    elseif selectedUpgradeMode == "10 Slot" then
        ...
        for _, slot in ipairs(allSlots) do if slot == selectedUpgradeMode then return {slot} end end
        return {}
    end
end
...
local function stopAutoUpgradeBrainrot()
    if upgradeBrainrotThread then task.cancel(upgradeBrainrotThread) upgradeBrainrotThread = nil end
end
...
local AutoUpgradeBrainrotToggle = AutoTab:Toggle({
    ...
    Callback = function(state)
        ...
        if state then startAutoUpgradeBrainrot() else stopAutoUpgradeBrainrot() end
        saveConfiguration()
    end
})
...
local BrainrotUpgradeDelaySlider = AutoTab:Slider({
    ...
    Value = {Min = 0.1, Max = 200, Default = 1},
    Callback = function(value)
        brainrotUpgradeDelay = value
        if autoUpgradeBrainrotEnabled then stopAutoUpgradeBrainrot() startAutoUpgradeBrainrot() end
        saveConfiguration()
    end
})
...
local function createBrainrotUpgradeDropdown()
    ...
    for _, slotNumber in ipairs(slots) do table.insert(dropdownValues, {Title = "Slot " .. slotNumber, Icon = "numeric-" .. slotNumber}) end
    
    if BrainrotUpgradeModeDropdown then BrainrotUpgradeModeDropdown:SetValues(dropdownValues)
    else
        BrainrotUpgradeModeDropdown = AutoTab:Dropdown({
            Title = "Brainrot Upgrade Mode",
            Values = dropdownValues,
            Value = "All",
            Callback = function(option) selectedUpgradeMode = option.Title saveConfiguration() end
        })
...
local function stopAutoPlaceBrainrot()
    if brainrotThread then task.cancel(brainrotThread) brainrotThread = nil end
end
...
local AutoPlaceBrainrotToggle = AutoTab:Toggle({
    ...
    Callback = function(state)
        ...
        if state then startAutoPlaceBrainrot() else stopAutoPlaceBrainrot() end
        saveConfiguration()
    end
})
...
local BrainrotPlaceDelaySlider = AutoTab:Slider({
    ...
    Value = {Min = 0.1, Max = 450, Default = 1},
    Callback = function(value)
        brainrotPlaceDelay = value
        if autoPlaceBrainrotEnabled then stopAutoPlaceBrainrot() startAutoPlaceBrainrot() end
        saveConfiguration()
    end
})
...
local function cleanupBatAura()
    if batAuraConnection then batAuraConnection:Disconnect() batAuraConnection = nil end
    if currentBat and originalHitboxSize then
        pcall(function() currentBat.Size = originalHitboxSize currentBat.Transparency = 0.5 end)
    end
    currentBat = nil originalHitboxSize = nil
end
...
local function enableBatAura()
    ...
    batAuraConnection = RunService.RenderStepped:Connect(function()
        ...
        pcall(function()
            ...
            if currentBat ~= hitbox then currentBat = hitbox originalHitboxSize = hitbox.Size lastHitbox = nil end
            local newTargetSize = Vector3.new(currentHitboxSize, currentHitboxSize, currentHitboxSize)
            if newTargetSize ~= targetSize then targetSize = newTargetSize lastHitbox = nil end
...
local BatAuraToggle = AutoTab:Toggle({
    ...
    Callback = function(state)
        ...
        if state then enableBatAura() else cleanupBatAura() end
        saveConfiguration()
    end
})
...
local function enableAutoBat()
    ...
    autoBatConnection = RunService.RenderStepped:Connect(function()
        ...
        pcall(function()
            ...
            if equippedBat then equippedBat:Activate()
            else
                local currentTime = tick()
                if currentTime - lastEquipAttempt >= EQUIP_COOLDOWN then
                    local backpackBat = LocalPlayer.Backpack:FindFirstChild("Basic Bat")
                    if backpackBat then humanoid:EquipTool(backpackBat) lastEquipAttempt = currentTime end
                end
            end
        end)
    end)
...
local AutoBatToggle = AutoTab:Toggle({
    ...
    Callback = function(state)
        ...
        if state then enableAutoBat() else cleanupAutoBat() end
        saveConfiguration()
    end
})
...
local function rebirth()
    pcall(function() ReplicatedStorage:WaitForChild("RemoteFunctions"):WaitForChild("Rebirth"):InvokeServer() end)
end

local function startAutoRebirth()
    if autoRebirthThread then task.cancel(autoRebirthThread) end
    autoRebirthThread = task.spawn(function()
        while isAutoRebirthEnabled do rebirth() task.wait(autoRebirthDelay) end
    end)
end
...
local AutoRebirthToggle = AutoTab:Toggle({
    Title = "Auto Rebirth",
    Desc = "Automatically rebirth to get rewards",
    Default = false,
    Callback = function(state)
...
local AutoRebirthDelaySlider = AutoTab:Slider({
    ...
    Value = {Min = 0.1, Max = 60, Default = 0.5},
    Callback = function(value)
        autoRebirthDelay = value
        if isAutoRebirthEnabled then stopAutoRebirth() startAutoRebirth() end
        saveConfiguration()
    end
})
...
local function removeESP(player)
    ...
    if esp then esp.box:Remove() esp.name:Remove() esp.highlight:Destroy() espObjects[player] = nil end
end
...
local ConnectionsESP = {ESPCelestial = nil, ESPLuckyBlock = nil}
...
local function toggleESPCelestial(state)
    ...
    if state then
        ConnectionsESP.ESPCelestial = RunService.RenderStepped:Connect(function()
            ...
            pcall(function()
                ...
                if root and not root:FindFirstChild("ESP_Celestial Brainrot") then createESPCelestial(root, "Celestial Brainrot") end
            end)
        end)
    else
        if ConnectionsESP.ESPCelestial then ConnectionsESP.ESPCelestial:Disconnect() ConnectionsESP.ESPCelestial = nil end
        clearESPCelestial()
    end
end
...
local function toggleESPLuckyBlock(state)
    ...
    if state then
        ConnectionsESP.ESPLuckyBlock = RunService.RenderStepped:Connect(function()
                ...
                for _, block in pairs(activeLuckyBlocks:GetChildren()) do
                    if block:IsA("Model") and string.find(block.Name, "NaturalLuckyBlock") then
                        ...
                        if rarity == "Secret" or rarity == "Cosmic" then
                            ...
                            if primary and not primary:FindFirstChild("ESP_LuckyBlock_" .. rarity) then createESPLuckyBlock(primary, rarity .. " Lucky Block") end
                        end
                    end
                end
            end)
        ...
        if ConnectionsESP.ESPLuckyBlock then ConnectionsESP.ESPLuckyBlock:Disconnect() ConnectionsESP.ESPLuckyBlock = nil end
        clearESPLuckyBlock()
    end
end
...
local EspCelestialToggle = VisualTab:Toggle({
    ...
    Default = false,
    Callback = function(state) toggleESPCelestial(state) saveConfiguration() end
})
...
local EspLuckyBlockToggle = VisualTab:Toggle({
    ...
    Default = false,
    Callback = function(state) toggleESPLuckyBlock(state) saveConfiguration() end
})
...
local UnlockZoomToggle = MiscTab:Toggle({
    ...
    Default = false,
    Callback = function(state)
        if state then LocalPlayer.CameraMaxZoomDistance = 500 else LocalPlayer.CameraMaxZoomDistance = 20 end
...
local InfJumpToggle = MiscTab:Toggle({
    ...
    Callback = function(state)
        ...
        if state or isJumpBoostEnabled then setupJumpFeatures() else cleanupJumpFeatures() end
        saveConfiguration()
    end
})
...
local JumpBoostToggle = MiscTab:Toggle({
    ...
    Callback = function(state)
        ...
        States.JumpPower = state -- [FIX] Sync with Anti Tsunami logic
        if state or isInfJumpEnabled then setupJumpFeatures() else cleanupJumpFeatures() end
        saveConfiguration()
    end
})
...
local JumpBoostSlider = MiscTab:Slider({
    ...
    Callback = function(value) jumpBoostPower = value saveConfiguration() end
})
...
local AutoSpinUFOToggle = EventsTab:Toggle({
    ...
    Callback = function(state)
        ...
        if state then startAutoSpinUFO() else stopAutoSpinUFO() end
        saveConfiguration()
    end
})
...
local UFOSpinDelaySlider = EventsTab:Slider({
    ...
    Value = {Min = 0.1, Max = 500, Default = 1},
    Callback = function(value)
        ufoSpinDelay = value
        if autoSpinUFOEnabled then stopAutoSpinUFO() startAutoSpinUFO() end
        saveConfiguration()
    end
})
...
local CreditsButton = CreditsTab:Button({
    ...
    Callback = function()
        ...
        WindUI:Notify({Title = "Discord Link Copied!", Content = "Discord invite copied to clipboard!", Duration = 3, Icon = "check",})
    end
})
...
local ThemeDropdown = SettingsTab:Dropdown({
    ...
    Values = {
        {Title = "Dark", Icon = "moon"}, {Title = "Light", Icon = "sun"}, {Title = "Purple Dream", Icon = "sparkles"},
        {Title = "Ocean Blue", Icon = "waves"}, {Title = "Forest Green", Icon = "tree-deciduous"}, {Title = "Crimson Red", Icon = "flame"},
        {Title = "Sunset Orange", Icon = "sunset"}, {Title = "Midnight Purple", Icon = "moon-star"}, {Title = "Cyan Glow", Icon = "zap"},
        {Title = "Rose Pink", Icon = "heart"}, {Title = "Golden Hour", Icon = "sun"}, {Title = "Neon Green", Icon = "zap-off"},
        {Title = "Electric Blue", Icon = "sparkle"}, {Title = "Custom", Icon = "palette"},
    },
    Value = "Dark",
...
local ThemeColorPicker = SettingsTab:Colorpicker({
    ...
    Callback = function(color)
        WindUI:AddTheme({
            Name = "Custom", Accent = color, Dialog = Color3.fromHex("#161616"), Outline = color, Text = Color3.fromHex("#FFFFFF"),
            Placeholder = Color3.fromHex("#7a7a7a"), Background = Color3.fromHex("#101010"), Button = Color3.fromHex("#52525b"), Icon = color
        })
...
local SaveConfigButton = SettingsTab:Button({
    ...
    Callback = function()
        saveConfiguration()
        WindUI:Notify({Title = "Configuration Saved", Content = "All your settings have been saved!", Duration = 3, Icon = "save",})
    end
})
...
local LoadConfigButton = SettingsTab:Button({
    ...
    Callback = function() loadConfiguration() end
})
...
myConfig:Register("GapDropdown", GapDropdown)
myConfig:Register("AntiTsunami", AntiTsunamiToggle) -- [FIX] Direct variable instead of GetElement
...
WindUI:Popup({
    ...
    Buttons = {
        ...
        {Title = "Join Discord", Icon = "users", Callback = function()
            setclipboard("https://discord.gg/XeBbhUnf")
            WindUI:Notify({Title = "Link Copied!", Content = "Discord invite copied to clipboard!", Duration = 3, Icon = "check",})
        end, Variant = "Primary"}
    }
})
...
if RarityDropdown and RarityDropdown.Value then
    ...
    if RARITY_TIERS[selectedTier] then selectedRarities = RARITY_TIERS[selectedTier]
    else selectedRarities = {selectedTier} end
end
...
if GapDropdown and GapDropdown.Value then CurrentGap = tonumber(GapDropdown.Value:match("%d+")) or 1 end
