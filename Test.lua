...
local function createESPForAnimal(animalData)
    ...
    pcall(function()
        if syn and syn.protect_gui then syn.protect_gui(espScreenGui)
        elseif gethui then espScreenGui.Parent = gethui() end
    end)
    ...
    genLabel.Text = animalData.genText or formatNumber(animalData.genValue or 0)
    ...
    rec.distConn = S.RunService.Heartbeat:Connect(function()
        ...
        if root and part and part.Parent then
            distanceLabel.Text = calculateDistance(root, part) .. "m"
        end
    end)

    rec.billboard = bb
    rec.screenGui = espScreenGui
