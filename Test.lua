...
local function getSideBounds(sideFolder)
    ...
    local halfSizeX = (maxX - minX) * 0.5
    local halfSizeY = (maxY - minY) * 0.5
...
-- ========================================
-- NEW getSafeOutsideDecorPos FUNCTION (2D)
-- ========================================
local function getSafeOutsideDecorPos(plot, targetPos, fromPos)
    local decorations = plot:FindFirstChild("Decorations")
    if not decorations then return targetPos end
    
    -- TUKAR DARI SIDE 3 KE SIDE 2
    local side2Folder = decorations:FindFirstChild("Side 2")
    if not side2Folder then return targetPos end
    
    local info = getSideBounds(side2Folder)
    if not info then return targetPos end
    
    local MARGIN = 1.6  -- Simple margin
    
    -- Check if target is inside bounds (X & Y only)
    local insideX = targetPos.X >= info.minX and targetPos.X <= info.maxX
    local insideY = targetPos.Y >= info.minY and targetPos.Y <= info.maxY
    
    if not (insideX and insideY) then
        return targetPos  -- Already outside
    end
    
    -- Calculate direction from player position (X & Y only)
    local srcX = fromPos and fromPos.X or targetPos.X
    local srcY = fromPos and fromPos.Y or targetPos.Y
    
    local dirX = srcX - info.centerX
    local dirY = srcY - info.centerY
    
    -- Normalize direction
    local magnitude = math.sqrt(dirX * dirX + dirY * dirY)
    if magnitude < 1e-3 then
        dirX, dirY = 1, 0  -- Default direction
    else
    ...
    if dirX ~= 0 then
        local boundX = (dirX > 0) and info.halfSizeX or -info.halfSizeX
        tx = boundX / dirX
    end
    
    if dirY ~= 0 then
        local boundY = (dirY > 0) and info.halfSizeY or -info.halfSizeY
        ty = boundY / dirY
    end
    
    local tHit = math.min(tx, ty)
    if tHit == math.huge then return targetPos end
    
    -- Simple calculation with margin
    local adjustedDistance = tHit + MARGIN
    
    -- Calculate final boundary position
    local finalX = info.centerX + dirX * adjustedDistance
    local finalY = info.centerY + dirY * adjustedDistance
    
    return Vector3.new(finalX, finalY, targetPos.Z)  -- Keep original Z
end
