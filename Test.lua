...
local espBestESP = nil
local espBestData = nil
local espBestLoop = nil
...
local function formatNumber(num)
    local value, suffix
    if num >= 1e12 then
        value = num / 1e12; suffix = "T/s"
    elseif num >= 1e9 then
        value = num / 1e9; suffix = "B/s"
    elseif num >= 1e6 then
        value = num / 1e6; suffix = "M/s"
    elseif num >= 1e3 then
        value = num / 1e3; suffix = "K/s"
    else
...
local function createEspBest(brainrotData)
    if not brainrotData or not brainrotData.model then return end
    pcall(function()
        if espBestESP then
            if espBestESP.highlight then espBestESP.highlight:Destroy() end
            if espBestESP.billboard then espBestESP.billboard:Destroy() end
            if espBestESP.updateConnection then espBestESP.updateConnection:Disconnect() end
            if espBestESP.beamUpdateConnection then espBestESP.beamUpdateConnection:Disconnect() end
            if espBestESP.beam then espBestESP.beam:Destroy() end
            if espBestESP.attachment0 then espBestESP.attachment0:Destroy() end
            if espBestESP.attachment1 then espBestESP.attachment1:Destroy() end
        end
        local espContainer = {}
        ...
        beamUpdateConnection = S.RunService.Heartbeat:Connect(function()
        ...
        updateConnection = S.RunService.Heartbeat:Connect(function()
        ...
        espBestESP = espContainer
        espBestData = brainrotData
    end)
end

local function checkEspBestExists()
    if not espBestData then return false end
    local exists = false
    pcall(function()
        if espBestData.model and espBestData.model.Parent then
            exists = true
        end
    end)
    return exists
...
local function updateEspBest()
    if espBestData and not checkEspBestExists() then
        if espBestESP then
            if espBestESP.highlight then espBestESP.highlight:Destroy() end
            if espBestESP.billboard then espBestESP.billboard:Destroy() end
            if espBestESP.updateConnection then espBestESP.updateConnection:Disconnect() end
    ...
    if newHighest then
        if not espBestData or newHighest.value > espBestData.value then
            createEspBest(newHighest)
        end
    end
end
...
local function enableEspBest()
    ...
    espBestEnabled = true
    updateEspBest()
    espBestLoop = task.spawn(function()
