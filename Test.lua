...
local function getTraitMultiplier(model)
    ...
    local traits = {}
    local ok, decoded = pcall(function() return game:GetService("HttpService"):JSONDecode(traitJson) end)
...
local function createHighestValueESP(brainrotData)
    ...
    pcall(function()
        ...
        -- Pet Name Label (Putih, Gotham Bold, Stroke Hitam)
        ...
        -- Gen Label (Hijau, Gotham Bold, Stroke Hitam)
        ...
        genLabel.Text = brainrotData.formattedValue
...
local function createTracerLine()
    ...
    local targetPart = highestValueData.model.PrimaryPart or highestValueData.model:FindFirstChild("HumanoidRootPart")
    if not targetPart then return false end
    
    pcall(function()
        removeTracerLine()
        
        tracerAttachment0 = Instance.new("Attachment"); tracerAttachment0.Parent = rootPart
        tracerAttachment1 = Instance.new("Attachment"); tracerAttachment1.Parent = targetPart
        ...
        tracerBeam.Width0 = 0.3; tracerBeam.Width1 = 0.3
        tracerBeam.Color = ColorSequence.new(Color3.fromRGB(255, 0, 0))
        tracerBeam.Parent = rootPart
        
        tracerConnection = S.RunService.Heartbeat:Connect(function()
...
local function updateHighestValueESP()
    if highestValueData and not pcall(function() return highestValueData.model.Parent end) then
    ...
    if newHighest then
        if not highestValueData or newHighest.value > highestValueData.value then
            ...
            if espBestEnabled then createTracerLine() end
        end
    end
end
...
local function enableESPBest()
    ...
    -- Auto recreate tracer on respawn
    espBestRespawnConn = player.CharacterAdded:Connect(function()
        if espBestEnabled and highestValueData then
            task.wait(1)
            createTracerLine()
        end
    end)
...
local function disableESPBest()
    ...
    if espBestRespawnConn then espBestRespawnConn:Disconnect(); espBestRespawnConn = nil end
end
