...
local function flyToBase()
    ...
    WindUI:Notify({
        Title = "Flying to Base", -- [DIUBAH]
        Content = "Flying to your base...", -- [DIUBAH]
        Duration = 2,
        Icon = "compass",
    })
...
local FlyToBaseButton = MainTab:Button({
    Title = "Fly to Base",
    Desc = "Flying to your base (Press B key for shortcut)", -- [DIUBAH]
    Callback = function()
        flyToBase()
    end
})
...
local function createBrainrotUpgradeDropdown()
    ...
    if BrainrotUpgradeModeDropdown then
        ...
        BrainrotUpgradeModeDropdown = AutoTab:Dropdown({
            Title = "Brainrot Upgrade Option", -- [DIUBAH] Tukar text di sini
...
local function toggleESPCelestial(state)
    ...
    if state then
        Connections.ESPCelestial = RunService.RenderStepped:Connect(function() -- [DIUBAH]
...
local function toggleESPLuckyBlock(state)
    ...
    if state then
        Connections.ESPLuckyBlock = RunService.RenderStepped:Connect(function() -- [DIUBAH]
...
-- [DIUBAH] Full Map Toggle -> Button
local FullMapButton = MiscTab:Button({
    Title = "Full Map",
    Desc = "Click to load a script to see the full map (May require rejoin to disable)",
    Callback = function()
        -- Use pcall for safety in case the link is down or the script is broken
        local success, errorMessage = pcall(function()
            loadstring(game:HttpGet("https://pastefy.app/XIwXl1GE/raw"))()
        end)
        
        if not success then
            WindUI:Notify({
                Title = "Error",
                Content = "Failed to load Full Map script: " .. tostring(errorMessage),
                Duration = 5,
                Icon = "alert",
            })
        else
            WindUI:Notify({
                Title = "Full Map Loaded",
                Content = "Full map script has been executed!",
                Duration = 3,
                Icon = "check",
            })
        end
...
-- myConfig:Register("FullMap", FullMapToggle) -- [DIUBAH] Dibuang kerana ia sudah jadi button
