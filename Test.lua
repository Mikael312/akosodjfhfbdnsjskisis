...
-- Esp Best & High Value Notify variables
...
espScreenGui.Name = "BrainrotESPWorld"
...
local function playNotifySound()
    pcall(function()
        notifySound:Stop()
        notifySound:Play()
        task.delay(5, function() notifySound:Stop() end)
    end)
end
...
-- ==================== ESP BEST FUNCTIONS ====================
local function formatBestNumber(num)
    local value, suffix
...
local function calculateBestDistance(part1, part2)
...
local function clearESPForUID(uid)
    ...
    if rec.beam then pcall(function() rec.beam:Destroy() end) end
    if rec.attachment0 then pcall(function() rec.attachment0:Destroy() end) end
    if rec.attachment1 then pcall(function() rec.attachment1:Destroy() end) end
    if rec.beamConn then pcall(function() rec.beamConn:Disconnect() end) end
    if rec.distConn then pcall(function() rec.distConn:Disconnect() end) end
    ESP_INSTANCES[uid] = nil
end

local function clearAllESP()
    for uid in pairs(ESP_INSTANCES) do clearESPForUID(uid) end
    ESP_BEST_UID = nil
end

local function createESPForAnimal(animalData)
    if not espBestEnabled then return end
    local uid = animalData.uid
    local part = getPodiumWorldPart(animalData)
    ...
    beam.Parent = part
    rec.beam = beam
    rec.attachment0 = attachment0
    rec.attachment1 = attachment1

    rec.beamConn = S.RunService.Heartbeat:Connect(function()
    ...
    nameLabel.BackgroundTransparency = 1
    nameLabel.Text = animalData.name or "Unknown"
    ...
    ownerLabel.Text = "@ " .. (animalData.owner or "Unknown")
    ...
    genLabel.Text = animalData.genText or formatBestNumber(animalData.genValue or 0)
    ...
    rec.distConn = S.RunService.Heartbeat:Connect(function()
        if not distanceLabel.Parent or not player.Character then return end
        local root = player.Character:FindFirstChild("HumanoidRootPart")
        if root and part and part.Parent then
            distanceLabel.Text = calculateBestDistance(root, part) .. "m"
        end
    end)
...
local function refreshAllESP()
    ...
    for _, animalData in ipairs(allAnimalsCache) do
        ...
        if plot and not isPlayerPlot(plot) then
            bestAnimal = animalData
            break
        end
    end

    local newBestUID = bestAnimal and bestAnimal.uid or nil

    if ESP_BEST_UID and ESP_BEST_UID ~= newBestUID then
        clearESPForUID(ESP_BEST_UID)
    end

    ESP_BEST_UID = newBestUID
    ...
    if bestAnimal then
        local rec = ESP_INSTANCES[ESP_BEST_UID]
        local part = getPodiumWorldPart(bestAnimal)
        if not part then
            clearESPForUID(ESP_BEST_UID)
        elseif not rec or rec.part ~= part then
            if rec then clearESPForUID(ESP_BEST_UID) end
            createESPForAnimal(bestAnimal)
        end
    else
        clearAllESP()
    end
    ...
    pcall(function()
        ...
        for slot, animalData in pairs(animalList) do
            if type(animalData) == "table" then
                ...
                local genText = formatBestNumber(genValue)
...
task.spawn(function()
    while true do
        task.wait(1)
        if highValueNotifyEnabled and highestAnimal then
            local plots = S.Workspace:FindFirstChild("Plots")
            local plot = plots and plots:FindFirstChild(highestAnimal.plot)
            if plot and not isPlayerPlot(plot) then
                local petKey = highestAnimal.name .. "_" .. highestAnimal.owner
