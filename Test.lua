--[[
    NIGHTMARE UI - INTEGRASI ESP PLAYERS, ESP BEST, BASE LINE, ANTI TURRET, AIMBOT, KICK STEAL, UNWALK ANIM, NEAREST, ANTI DEBUFF, ANTI RDOLL, XRAY BASE, FPS BOOST & ESP TIMER
]]
...
-- ==================== SMART STEAL (NEAREST) VARIABLES ====================
local smartStealEnabled = false
    ...
    for targetPlayer, espData in pairs(espObjects) do
        if targetPlayer and targetPlayer.Parent and espData.character and espData.character.Parent then
            ...
            if rootPart then
                ...
                espData.itemLabel.TextColor3 = (equippedItem ~= "None" and Color3.fromRGB(255, 100, 100) or Color3.fromRGB(255, 255, 100))
            else
...
local function enableESPPlayers()
    ...
    for _, targetPlayer in pairs(Players:GetPlayers()) do
        if targetPlayer ~= player and targetPlayer.Character then createESP(targetPlayer) end
    end
...
local function disableESPPlayers()
    ...
    for targetPlayer, _ in pairs(espObjects) do removeESP(targetPlayer) end
...
local function refreshTracerLine()
    if not espBestEnabled or not highestValueData then removeTracerLine(); return end
    removeTracerLine()
    createTracerLine()
end
...
local function updateHighestValueESP()
    ...
    if newHighest then
        if not highestValueData or newHighest.value > highestValueData.value then
            ...
            if espBestEnabled then refreshTracerLine() end
            return newHighest
        end
    end
        ...
        if plot:IsA("Model") or plot:IsA("Folder") then
            ...
            if plotSign and plotSign:FindFirstChild("SurfaceGui") then
                ...
                if surfaceGui:FindFirstChild("Frame") and surfaceGui.Frame:FindFirstChild("TextLabel") then
                    ...
                    if plotSignText == playerBaseName then print("✅ Found player's plot:", plot.Name); return plot, plotSign end
                end
            end
        end
    end
...
local function isMySentry(sentryName) return string.find(sentryName, myUserId) ~= nil end
local function isOwnedByPlayer(desc) if isMySentry(desc.Name) then return true end return false end

local function findBat()
    local tool = nil
    pcall(function()
        tool = player.Backpack:FindFirstChild("Bat")
        if not tool and player.Character then tool = player.Character:FindFirstChild("Bat") end
    end)
    return tool
end
...
local function equipBat()
    ...
    if bat and bat.Parent == player.Backpack then
        pcall(function() player.Character.Humanoid:EquipTool(bat) end)
        return true
    end
...
local function unequipBat()
    local bat = findBat()
    if bat and bat.Parent == player.Character then pcall(function() player.Character.Humanoid:UnequipTools() end) end
end
...
local function sentryExists(desc)
    ...
    pcall(function() stillExists = desc.Parent ~= nil and desc:IsDescendantOf(Workspace) end)
    return stillExists
end
...
local function destroySentry(desc)
    ...
    if isOwnedByPlayer(desc) then print("[🛡️] Skipping own sentry: " .. desc.Name); processedSentries[desc] = true; return end
    ...
    local bat = findBat()
    if not bat then warn("[⚠️] Bat not found!"); activeSentries[desc] = nil; return end
    ...
    followConnection = RunService.RenderStepped:Connect(function()
        if not running or not sentryEnabled or not sentryExists(desc) then if followConnection then followConnection:Disconnect(); followConnections[desc] = nil end; return end
        updateSentryPosition(desc)
    end)
    followConnections[desc] = followConnection
    task.spawn(function()
        while running and sentryEnabled and sentryExists(desc) do equipBat(); task.wait(0.05); if not running or not sentryExists(desc) then break end; unequipBat(); task.wait(0.05) end
    end)
    task.spawn(function()
        task.wait(0.1)
        local spamConnection
        spamConnection = RunService.Heartbeat:Connect(function()
            if not sentryEnabled or not sentryExists(desc) then running = false
                if spamConnection then spamConnection:Disconnect() end
                if destroyConnection then destroyConnection:Disconnect() end
                if followConnections[desc] then followConnections[desc]:Disconnect(); followConnections[desc] = nil end
                unequipBat(); activeSentries[desc] = nil
            ...
            if currentBat and currentBat.Parent == player.Character then for i = 1, 12 do if currentBat.Parent == player.Character and sentryExists(desc) then currentBat:Activate(); hitCount = hitCount + 1 else break end end end
        end)
    end)
end
        ...
        for _, desc in pairs(Workspace:GetDescendants()) do
            if sentryEnabled and (desc:IsA("Model") or desc:IsA("BasePart")) then
                if string.find(desc.Name:lower(), "sentry") then
                    if isSentryPlaced(desc) and not processedSentries[desc] and not isOwnedByPlayer(desc) then foundCount = foundCount + 1; updateSentryPosition(desc); destroySentry(desc); task.wait(0.1) end
                end
            end
        end
    end)
...
local function startSentryWatch()
    ...
    sentryConn = Workspace.DescendantAdded:Connect(function(desc)
        ...
        local char = player.Character
        if not char or not char:FindFirstChild("HumanoidRootPart") then return end
        if isOwnedByPlayer(desc) then print("[🛡️] Detected own sentry: " .. desc.Name); processedSentries[desc] = true; return end
        task.wait(0.5)
        if not isSentryPlaced(desc) then
            print("[⏳] Waiting for sentry to be placed: " .. desc.Name)
            task.spawn(function()
                local waitTime = 0
                while waitTime < 10 and not isSentryPlaced(desc) and sentryExists(desc) and sentryEnabled do task.wait(0.5); waitTime = waitTime + 0.5 end
                if isSentryPlaced(desc) and sentryExists(desc) and sentryEnabled then print("[✅] Sentry placed, attacking: " .. desc.Name); updateSentryPosition(desc); destroySentry(desc) end
            end)
            return
        end
        task.wait(4.1)
    ...
    scanConn = task.spawn(function() while sentryEnabled do scanExistingSentries(); task.wait(5) end end)
...
local function stopSentryWatch()
    ...
    if scanConn then task.cancel(scanConn); scanConn = nil end
    for _, conn in pairs(followConnections) do if conn then conn:Disconnect() end end
    followConnections = {}
    activeSentries = {}
    processedSentries = {}
...
local function autoLaserWorker()
    while autoLaserEnabled do
        ...
        local t0 = tick()
        while tick() - t0 < 0.6 do if not autoLaserEnabled then break end; RunService.Heartbeat:Wait() end
    end
end
...
local function kickPlayer()
    local success = pcall(function() player:Kick("Steal Success!") end)
...
local function startMonitoring()
    ...
    monitoringLoop = RunService.Heartbeat:Connect(function()
        ...
        if currentStealCount > lastStealCount then
            ...
            isMonitoring = false
            if monitoringLoop then monitoringLoop:Disconnect(); monitoringLoop = nil end
            task.wait(0.1)
            kickPlayer()
        end
...
local function setupNoWalkAnimation(character)
    ...
    local renderSteppedConnection = RunService.RenderStepped:Connect(function() stopAllAnimations() end)
...
local function disableUnwalkAnim()
    ...
    for _, connection in pairs(unwalkAnimConnections) do if connection then connection:Disconnect() end end
    unwalkAnimConnections = {}
    print("❌ Unwalk Animation Disabled")
end

-- ==================== SMART STEAL (NEAREST) FUNCTIONS ====================
local function getSafeCoreGuiParent()
    ...
    if syn and syn.protect_gui then
        ...
        protectedGui.Name = "StealBar_Protected"
...
local function createStealUI()
    if screenGui then return end -- UI already exists
    ...
    modeButton.MouseButton1Click:Connect(function()
        if stealMode == "bestgen" then
            ...
            print("🔄 Mode switched to: NEAREST")
        else
            stealMode = "bestgen"
            modeButton.Text = "MODE: BEST GEN"
            modeButton.BackgroundColor3 = Color3.fromRGB(180, 30, 30)
            modeStroke.Color = Color3.fromRGB(220, 50, 50)
            print("🔄 Mode switched to: BEST GEN")
        end
    end)
end

local function destroyStealUI()
    if screenGui then
        screenGui:Destroy()
        screenGui = nil
        mainFrame = nil
...
local function animateFill()
    ...
    tween:Play()
    task.spawn(function()
        for i = 0, 100 do percentLabel.Text = i .. "%"; task.wait(0.7 / 100) end
    end)
...
local function isMyBaseAnimal(animalData)
    ...
    if channel then
        ...
        if owner then
            ...
            elseif typeof(owner) == "table" and owner.UserId then return owner.UserId == player.UserId
            elseif typeof(owner) == "Instance" then return owner == player end
        end
    end
...
local function getBestStealableAnimal()
    for _, animal in ipairs(allAnimalsCache) do if not isMyBaseAnimal(animal) then return animal end end
    return nil
end
...
local function getTargetAnimal()
    if stealMode == "nearest" then return getNearestAnimal() else return getBestStealableAnimal() end
end
...
local function updateUseItemEventHandler()
    ...
    if not antiBeeEnabled and not antiBoogieEnabled then
        if isEventHandlerActive then
            ...
            if unifiedConnection then unifiedConnection:Disconnect(); unifiedConnection = nil end
            for _, conn in pairs(originalConnections) do pcall(function() conn:Enable() end) end
            originalConnections = {}
    ...
    if (antiBeeEnabled or antiBoogieEnabled) and not isEventHandlerActive then
        ...
        for i, v in pairs(getconnections(Event.OnClientEvent)) do table.insert(originalConnections, v); pcall(function() v:Disable() end) end
        unifiedConnection = Event.OnClientEvent:Connect(function(Action, ...)
            if antiBeeEnabled and Action == "Bee Attack" then print("🐝 Blocked Bee Attack!"); return end
            if antiBoogieEnabled and Action == "Boogie" then print("🕺 Blocked Boogie Bomb!"); return end
        end)
...
local function setupInstantAnimationBlocker()
    ...
    animationPlayedConnection = animator.AnimationPlayed:Connect(function(track)
        if track and track.Animation then
            if tostring(track.Animation.AnimationId):gsub("%D", "") == BOOGIE_ANIMATION_ID then track:Stop(0); track:Destroy(); print("⚡ INSTANT BLOCK: Boogie animation destroyed!") end
        end
    end)
end
...
local function enableContinuousMonitoring()
    ...
    heartbeatConnection = RunService.Heartbeat:Connect(function()
        ...
        lastCheck = now
        pcall(function()
            if Lighting:FindFirstChild("DiscoEffect") then Lighting.DiscoEffect:Destroy() end
            for _, v in pairs(Lighting:GetChildren()) do if v:IsA("BlurEffect") then v:Destroy() end end
            ...
            if boogieScript then
                ...
                if boom and boom:IsA("Sound") and boom.Playing then boom:Stop() end
            end
        end)
    end)
end
...
local function toggleAntiBee(state)
    ...
    if antiBeeEnabled then print("✅ Anti Bee Enabled") else print("❌ Anti Bee Disabled") end
end

local function toggleAntiBoogie(state)
    antiBoogieEnabled = state
    if antiBoogieEnabled then setupInstantAnimationBlocker(); enableContinuousMonitoring(); print("✅ Anti Boogie Bomb: ENABLED")
    else
...
local function toggleAntiDebuff(state)
    ...
    if state then print("✅ Anti Debuff Enabled") else print("❌ Anti Debuff Disabled") end
end
...
local function disconnectAll()
    for _, conn in ipairs(ragdollConnections) do if typeof(conn) == "RBXScriptConnection" then pcall(function() conn:Disconnect() end) end end
    ragdollConnections = {}
end
local function isRagdolled()
    ...
    if endTime then local now = workspace:GetServerTimeNow(); if (endTime - now) > 0 then return true end end
    return false
end
local function removeRagdollConstraints()
    ...
    for _, descendant in ipairs(cachedCharData.character:GetDescendants()) do
        if descendant:IsA("BallSocketConstraint") or (descendant:IsA("Attachment") and descendant.Name:find("RagdollAttachment")) then pcall(function() descendant:Destroy(); removed = true end) end
    end
    return removed
end
local function forceExitRagdoll()
    if not cachedCharData.humanoid or not cachedCharData.root then return end
    local hum = cachedCharData.humanoid; local root = cachedCharData.root
    pcall(function() local now = workspace:GetServerTimeNow(); player:SetAttribute("RagdollEndTime", now) end)
...
local function antiRagdollLoop()
    while antiRagdollActive and cachedCharData.humanoid do task.wait(); if isRagdolled() then removeRagdollConstraints(); forceExitRagdoll() end end
end
local function setupCameraBinding()
    ...
    local conn = RunService.RenderStepped:Connect(function()
        ...
        if cam and cachedCharData.humanoid and cam.CameraSubject ~= cachedCharData.humanoid then cam.CameraSubject = cachedCharData.humanoid end
    end)
...
function ANTI_RAGDOLL.Enable()
    if antiRagdollActive then warn("[Anti-Ragdoll] Already enabled!"); return end
...
local function tryApplyInvisibleWalls()
    ...
    local plots = workspace:FindFirstChild("Plots")
    if not plots or #plots:GetChildren() == 0 then print("❌ No plots found for Xray Base"); return end
    print("🔍 Applying Xray to base walls...")
    local processedCount = 0
    for _, plot in pairs(plots:GetChildren()) do
        for _, obj in pairs(plot:GetDescendants()) do
            if obj:IsA("BasePart") and obj.Anchored and obj.CanCollide and isBaseWall(obj) then
                if not originalTransparency[obj] then originalTransparency[obj] = obj.LocalTransparencyModifier; obj.LocalTransparencyModifier = 0.85; processedCount = processedCount + 1 end
            end
        end
    end
...
local function cleanupRemovedParts()
    for obj, _ in pairs(originalTransparency) do if not obj or not obj.Parent then originalTransparency[obj] = nil end end
end
local function enableXrayBase()
    if xrayBaseEnabled then print("⚠️ Xray Base already enabled"); return end
    xrayBaseEnabled = true; invisibleWallsLoaded = false
    cleanupRemovedParts()
    ...
    xrayBaseConnection = workspace.DescendantAdded:Connect(function(obj)
        ...
        if isBaseWall(obj) and obj:IsA("BasePart") and obj.Anchored and obj.CanCollide then
            if not originalTransparency[obj] then originalTransparency[obj] = obj.LocalTransparencyModifier; obj.LocalTransparencyModifier = 0.85; print("🔍 Applied Xray to new base wall: " .. obj.Name) end
        end
    end)
    print("✅ Xray Base Enabled")
end
local function disableXrayBase()
    if not xrayBaseEnabled then print("⚠️ Xray Base already disabled"); return end
    xrayBaseEnabled = false; invisibleWallsLoaded = false
    if xrayBaseConnection then xrayBaseConnection:Disconnect(); xrayBaseConnection = nil end
    local restoredCount = 0
    for obj, value in pairs(originalTransparency) do if obj and obj.Parent then pcall(function() obj.LocalTransparencyModifier = value; restoredCount = restoredCount + 1 end) end end
...
-- ==================== FPS BOOST FUNCTIONS ====================
local function addThread(func) local t = task.spawn(func); table.insert(optimizerThreads, t); return t end
local function addConnection(conn) table.insert(optimizerConnections, conn); return conn end
local function storeOriginalSettings()
    pcall(function()
        originalSettings = {
            streamingEnabled = workspace.StreamingEnabled, streamingMinRadius = workspace.StreamingMinRadius, streamingTargetRadius = workspace.StreamingTargetRadius,
            qualityLevel = settings().Rendering.QualityLevel, meshPartDetailLevel = settings().Rendering.MeshPartDetailLevel,
            globalShadows = game.Lighting.GlobalShadows, brightness = game.Lighting.Brightness, fogEnd = game.Lighting.FogEnd, technology = game.Lighting.Technology,
            environmentDiffuseScale = game.Lighting.EnvironmentDiffuseScale, environmentSpecularScale = game.Lighting.EnvironmentSpecularScale,
            decoration = workspace.Terrain.Decoration, waterWaveSize = workspace.Terrain.WaterWaveSize, waterWaveSpeed = workspace.Terrain.WaterWaveSpeed,
            waterReflectance = workspace.Terrain.WaterReflectance, waterTransparency = workspace.Terrain.WaterTransparency,
        }
    end)
end
local PERFORMANCE_FFLAGS = {
    ...
    ["DFIntHttpCurlConnectionCacheSize"] = 134217728, ["DFIntCSGLevelOfDetailSwitchingDistance"] = 0, ["DFIntCSGLevelOfDetailSwitchingDistanceL12"] = 0,
    ["DFIntCSGLevelOfDetailSwitchingDistanceL23"] = 0, ["DFIntCSGLevelOfDetailSwitchingDistanceL34"] = 0, ["FFlagEnableInGameMenuChromeABTest3"] = false,
    ["FFlagEnableInGameMenuModernization"] = false, ["FFlagEnableReportAbuseMenuRoactABTest2"] = false, ["FFlagDisableNewIGMinDUA"] = true,
    ["FFlagEnableV3MenuABTest3"] = false, ["FIntRobloxGuiBlurIntensity"] = 0, ["DFIntTimestepArbiterThresholdCFLThou"] = 10,
    ["DFIntTextureQualityOverride"] = 1, ["DFIntPerformanceControlTextureQualityBestUtility"] = 1, ["DFIntTexturePoolSizeMB"] = 64,
    ["DFIntMaxFrameBufferSize"] = 1, ["FFlagDebugDisableParticleRendering"] = false, ["DFIntParticleMaxCount"] = 100,
    ["FFlagEnableWaterReflections"] = false, ["DFIntWaterReflectionQuality"] = 0,
}
...
local function optimizeCharacter(char)
    ...
    task.spawn(function()
        task.wait(0.5)
        pcall(function()
            for _, part in ipairs(char:GetDescendants()) do pcall(function()
...
local function enableFpsBoost()
    ...
    pcall(applyFFlags)
    pcall(function()
        workspace.StreamingEnabled = true; workspace.StreamingMinRadius = 64; workspace.StreamingTargetRadius = 256; workspace.StreamingIntegrityMode = Enum.StreamingIntegrityMode.MinimumRadiusPause
    end)
    pcall(function()
        ...
        game.Lighting.GlobalShadows = false; game.Lighting.Brightness = 3; game.Lighting.FogEnd = 9e9; game.Lighting.Technology = Enum.Technology.Legacy
        game.Lighting.EnvironmentDiffuseScale = 0; game.Lighting.EnvironmentSpecularScale = 0
        for _, effect in ipairs(game.Lighting:GetChildren()) do if effect:IsA("PostEffect") then pcall(function() effect.Enabled = false; effect:Destroy() end) end end
    end)
    pcall(function()
    ...
    for _, p in ipairs(Players:GetPlayers()) do if p.Character then optimizeCharacter(p.Character) end
        addConnection(p.CharacterAdded:Connect(function(char) if getgenv().OPTIMIZER_ACTIVE then optimizeCharacter(char) end end))
    end
    addConnection(Players.PlayerAdded:Connect(function(p)
        addConnection(p.CharacterAdded:Connect(function(char) if getgenv().OPTIMIZER_ACTIVE then optimizeCharacter(char) end end))
    end))
    pcall(function() setfpscap(999) end)
    pcall(function() local cam = workspace.CurrentCamera; cam.FieldOfView = 70 end)
    print("✅ Fps Boost Enabled")
end
local function disableFpsBoost()
    if not fpsBoostEnabled then return end
    fpsBoostEnabled = false; getgenv().OPTIMIZER_ACTIVE = false
    for _, thread in ipairs(optimizerThreads) do pcall(function() task.cancel(thread) end) end; optimizerThreads = {}
    for _, conn in ipairs(optimizerConnections) do pcall(function() conn:Disconnect() end) end; optimizerConnections = {}
    pcall(function()
        workspace.StreamingEnabled = originalSettings.streamingEnabled or true; workspace.StreamingMinRadius = originalSettings.streamingMinRadius or 64; workspace.StreamingTargetRadius = originalSettings.streamingTargetRadius or 1024
        settings().Rendering.QualityLevel = originalSettings.qualityLevel or Enum.QualityLevel.Automatic; settings().Rendering.MeshPartDetailLevel = originalSettings.meshPartDetailLevel or Enum.MeshPartDetailLevel.DistanceBased
        game.Lighting.GlobalShadows = originalSettings.globalShadows ~= false; game.Lighting.Brightness = originalSettings.brightness or 1; game.Lighting.FogEnd = originalSettings.fogEnd or 100000
        game.Lighting.Technology = originalSettings.technology or Enum.Technology.ShadowMap; game.Lighting.EnvironmentDiffuseScale = originalSettings.environmentDiffuseScale or 1
        game.Lighting.EnvironmentSpecularScale = originalSettings.environmentSpecularScale or 1
        workspace.Terrain.WaterWaveSize = originalSettings.waterWaveSize or 0.15; workspace.Terrain.WaterWaveSpeed = originalSettings.waterWaveSpeed or 10
        workspace.Terrain.WaterReflectance = originalSettings.waterReflectance or 1; workspace.Terrain.WaterTransparency = originalSettings.waterTransparency or 0.3
...
local function enableTimerESP()
    ...
    task.spawn(function() while timerEspEnabled do pcall(scanAndConnect); task.wait(5) end end)
...
local function disableTimerESP()
    ...
    for _, connections in pairs(timerEspConnections) do for _, connection in ipairs(connections) do connection:Disconnect() end end
    timerEspConnections = {}
    print("❌ Timer ESP disabled")
end

-- ==================== TOGGLE FUNCTIONS FOR UI ====================
local function toggleEspPlayers(state) if state then enableESPPlayers() else disableESPPlayers() end end
local function toggleEspBest(state) if state then enableESPBest() else disableESPBest() end end
local function toggleBaseLine(state) if state then enableBaseLine() else disableBaseLine() end end
local function toggleAntiTurret(state) if state then enableAntiTurret() else disableAntiTurret() end end
local function toggleAimbot(state) if state then enableAimbot() else disableAimbot() end end
local function toggleKickSteal(state) if state then enableKickSteal() else disableKickSteal() end end
local function toggleUnwalkAnim(state) if state then enableUnwalkAnim() else disableUnwalkAnim() end end
local function toggleSmartSteal(state)
    smartStealEnabled = state
    if state then
        createStealUI()
        startAutoSteal()
        print("✅ Nearest Enabled")
    else
        stopAutoSteal()
        destroyStealUI()
        print("❌ Nearest Disabled")
    end
end
local function toggleAntiRagdoll(state) if state then ANTI_RAGDOLL.Enable() else ANTI_RAGDOLL.Disable() end end
local function toggleXrayBase(state) if state then enableXrayBase() else disableXrayBase() end end
local function toggleFpsBoost(state) if state then enableFpsBoost() else disableFpsBoost() end end
local function toggleEspTimer(state) if state then enableTimerESP() else disableTimerESP() end end

-- ==================== PLAYER EVENT HANDLERS ====================
Players.PlayerAdded:Connect(function(targetPlayer)
    targetPlayer.CharacterAdded:Connect(function(character) task.wait(1); if espPlayersEnabled and targetPlayer ~= player then createESP(targetPlayer) end end)
end)
Players.PlayerRemoving:Connect(function(targetPlayer) removeESP(targetPlayer) end)
for _, targetPlayer in pairs(Players:GetPlayers()) do
    if targetPlayer ~= player then
        targetPlayer.CharacterAdded:Connect(function(character) task.wait(1); if espPlayersEnabled then createESP(targetPlayer) end end)
    end
end

player.CharacterAdded:Connect(function(newCharacter)
    task.wait(1)
    if espBestEnabled then updateHighestValueESP(); if highestValueData then createTracerLine() end end
    if baseLineEnabled then pcall(stopPlotLine); task.wait(0.5); pcall(createPlotLine) end
    if sentryEnabled then stopSentryWatch(); sentryEnabled = false; task.wait(0.5); sentryEnabled = true; startSentryWatch() end
    if autoLaserEnabled then autoLaserEnabled = false; task.wait(0.5); autoLaserEnabled = true; if autoLaserThread then task.cancel(autoLaserThread) end; autoLaserThread = task.spawn(autoLaserWorker) end
    if isMonitoring then stopMonitoring(); task.wait(0.5); startMonitoring() end
    if unwalkAnimEnabled then setupNoWalkAnimation(newCharacter) end
    if antiBoogieEnabled then setupInstantAnimationBlocker(); print("🔄 Reloaded animation blocker after respawn") end
    if antiRagdollActive then ANTI_RAGDOLL.Disable(); task.wait(0.5); ANTI_RAGDOLL.Enable() end
    if xrayBaseEnabled then invisibleWallsLoaded = false; tryApplyInvisibleWalls() end
    if fpsBoostEnabled then for _, p in ipairs(Players:GetPlayers()) do if p.Character then optimizeCharacter(p.Character) end end end
end)

player.CharacterRemoving:Connect(function() pcall(stopPlotLine) end)

-- ==================== AUTO SCAN PLOTS ====================
task.spawn(function() while task.wait(5) do scanAllPlots() end end)
...
Nightmare:AddToggleRow("Unwalk Anim", toggleUnwalkAnim, "Nearest", toggleSmartSteal)
