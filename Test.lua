--[[
    NIGHTMARE UI - INTEGRASI ESP PLAYERS, ESP BEST, BASE LINE, ANTI TURRET, AIMBOT, KICK STEAL, UNWALK ANIM, NEAREST STEAL, ANTI DEBUFF, ANTI RDOLL, XRAY BASE, FPS BOOST & ESP TIMER
]]
...
local function destroyStealUI()
    if not stealUI then return end
...
local function animateFill()
    if not stealUI or stealUI.isAnimating then return end 
    stealUI.isAnimating = true

    local tweenInfo = TweenInfo.new(0.7, Enum.EasingStyle.Linear)
    local tween = TweenService:Create(stealUI.fillBar, tweenInfo, {Size = UDim2.new(1, 0, 1, 0)})
    
    tween:Play()
    
    task.spawn(function()
        for i = 0, 100 do
            if stealUI and stealUI.percentLabel then
                stealUI.percentLabel.Text = i .. "%"
            end
            task.wait(0.7 / 100)
        end
    end)
    ...
    if stealUI and stealUI.fillBar then
        stealUI.fillBar.Size = UDim2.new(0, 0, 1, 0)
    end
    if stealUI and stealUI.percentLabel then
        stealUI.percentLabel.Text = ""
    end
    stealUI.isAnimating = false
end
...
local function startAutoSteal()
    ...
    stealConnection = RunService.Heartbeat:Connect(function()
        ...
        if not targetAnimal or not stealUI then
            if stealUI then
                stealUI.titleLabel.Text = "WAITING FOR STEAL"
                stealUI.studLabel.Text = "0.0 studs"
                stealUI.moneyLabel.Text = "$0.0M/s"
            end
            return
        end
        ...
        if stealUI then
            stealUI.titleLabel.Text = string.upper(targetAnimal.name)
            stealUI.studLabel.Text = string.format("%.1f studs", dist)
            stealUI.moneyLabel.Text = string.format("$%.1fM/s", targetAnimal.genValue / 1000000)
        end
