--[[
    ...
    - Replaced "Esp Best" with "Brainrot ESP V2"
]]
...
-- Brainrot ESP V2 variables (NEW - REPLACING Esp Best)
...
-- ==================== BRAINROT ESP V2 FUNCTION (NEW - REPLACING Esp Best) ====================
-- Utility function to parse numbers with suffixes
local function parseNumber(txt)
    local cleaned = txt:gsub(',', ''):gsub('%s+', ''):gsub('%$', '')
    local num, suffix = cleaned:match('(%d+%.?%d*)([kKmMbBtT]?)%s')
    if not num then
        num, suffix = cleaned:match('(%d+%.?%d*)([kKmMbBtT]?)')
    end
    if not num then
        return nil
    end
...
local function findHighestBrainrot()
    ...
    local totalLabelsScanned = 0
    local totalPetsFound = 0
    
    print("========== SCANNING ALL PLOTS ==========")
        ...
        if not isPlayerPlot(plot) then
            ...
            for _, obj in pairs(plot:GetDescendants()) do
                if obj:IsA("TextLabel") then
                    totalLabelsScanned = totalLabelsScanned + 1
                    ...
                    if txt:find("/") and txt:lower():find("s") then
                        pcall(function()
                            ...
                            local genText = txt
                            local value = parseNumber(genText)
                            
                            if value <= 0 then return end
                            
                            totalPetsFound = totalPetsFound + 1
                            
                            print(string.format("🔍 Plot: %s | Pet: %s | Price: %s | Value: %.0f", 
                                plot.Name, petName, genText, value))
                            
                            if value > highest.value then
                                print(string.format("   ✅ NEW HIGHEST! (%.0f > %.0f)", value, highest.value))
    ...
    print(string.format("📊 SCAN STATS: Plots: %d | Labels: %d | Pets: %d", 
        totalPlotsScanned, totalLabelsScanned, totalPetsFound))
    
    if highest.value > 0 then
        print(string.format("========== FINAL: %s at Plot %s (Value: %.0f) ==========", 
            highest.petName, highest.plotName, highest.value))
    else
        print("========== NO PETS FOUND ==========")
    end
...
local function createHighestValueESP(brainrotData)
    ...
    pcall(function()
        ...
        -- RED OUTLINE for PODIUM (plot platform)
        ...
        priceLabel.TextColor3 = Color3.fromRGB(255, 0, 0)
...
local function checkPetExists()
    ...
    local exists = false
    pcall(function()
        local genLabel = highestValueData.generationLabel
        local nameLabel = highestValueData.nameLabel
        
        if genLabel and genLabel.Parent and nameLabel and nameLabel.Parent then
            if genLabel.Text ~= "" and genLabel.Text ~= "N/A" and nameLabel.Text ~= "" then
                exists = true
            end
        end
    end)
...
local function createTracerLine()
    ...
    local targetPart = highestValueData.model.PrimaryPart or highestValueData.model:FindFirstChildWhichIsA('BasePart')
