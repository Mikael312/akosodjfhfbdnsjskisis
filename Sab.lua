...
local function parsePrice(text)
    ...
    if suffix then suffix = suffix:lower(); if suffix == "k" then number = number * 1e3; elseif suffix == "m" then number = number * 1e6; elseif suffix == "b" then number = number * 1e9; end end
    return number
end
                    ...
                    pcall(function()
                        ...
                        if priceLabel.Text ~= "" and priceLabel.Text ~= "N/A" then
                            ...
                            if value > highest.value then
                                ...
                                highest = {
                                    plot = plot, plotName = plot.Name, podiumNumber = i, petName = nameLabel and nameLabel.Text or "Unknown",
                                    price = priceLabel.Text, priceValue = value, teleportPart = tpPart, position = tpPart.Position,
                                    rarity = overhead.Rarity.Text, mutation = overhead.Mutation.Text, value = value
                                }
                            end
                        end
                    end)
...
local function createHighestValueESP(brainrotData)
    ...
    pcall(function()
        if highestValueESP then if highestValueESP.highlight then highestValueESP.highlight:Destroy() end; if highestValueESP.nameLabel then highestValueESP.nameLabel:Destroy() end end
        local espContainer = {}; local part = brainrotData.teleportPart
        local highlight = Instance.new("Highlight", part)
        highlight.FillColor = Color3.fromRGB(0, 255, 255); highlight.OutlineColor = Color3.fromRGB(0, 255, 255)
        highlight.FillTransparency = 0.7; highlight.OutlineTransparency = 0; highlight.DepthMode = Enum.HighlightDepthMode.AlwaysOnTop
        espContainer.highlight = highlight
        local billboard = Instance.new("BillboardGui", part)
        billboard.Size = UDim2.new(0, 200, 0, 70); billboard.StudsOffset = Vector3.new(0, 8, 0); billboard.AlwaysOnTop = true
        ...
        petNameLabel.Size = UDim2.new(1, 0, 0.5, 0); petNameLabel.BackgroundTransparency = 1; petNameLabel.Text = brainrotData.petName or "Unknown"
        petNameLabel.TextColor3 = Color3.fromRGB(255, 150, 255); petNameLabel.TextStrokeTransparency = 0; petNameLabel.TextScaled = true; petNameLabel.Font = Enum.Font.GothamBold
        local priceLabel = Instance.new("TextLabel", container)
        priceLabel.Size = UDim2.new(1, 0, 0.5, 0); priceLabel.Position = UDim2.new(0, 0, 0.5, 0); priceLabel.BackgroundTransparency = 1
        priceLabel.Text = brainrotData.price or ""; priceLabel.TextColor3 = Color3.fromRGB(0, 255, 0); priceLabel.TextStrokeTransparency = 0
        priceLabel.TextScaled = true; priceLabel.Font = Enum.Font.GothamBold
        espContainer.nameLabel = billboard; highestValueESP = espContainer; highestValueData = brainrotData
    end)
end

local function updateHighestValueESP()
    local newHighest = findHighestBrainrot()
    if newHighest then if not highestValueData or newHighest.priceValue > highestValueData.priceValue then createHighestValueESP(newHighest); return newHighest end end
    return highestValueData
end

local function removeHighestValueESP()
    if highestValueESP then pcall(function() if highestValueESP.highlight then highestValueESP.highlight:Destroy() end; if highestValueESP.nameLabel then highestValueESP.nameLabel:Destroy() end end); highestValueESP = nil; highestValueData = nil end
end
                    ...
                    pcall(function()
                        ...
                        if plot and plot.Parent then
                            ...
                            if podiums then
                                ...
                                if podium then
                                    local overhead = podium.Base.Spawn.Attachment.AnimalOverhead; local nameLabel = overhead.DisplayName
                                    if nameLabel and nameLabel.Text ~= "" then petStillExists = true end
                                end
                            end
                        end
                    end)
                    if not petStillExists then print("⚠️ Current pet removed (player left), searching for new highest value..."); removeHighestValueESP(); highestValueData = nil end
                end
                updateHighestValueESP()
            end
        end)
...
local function toggleEspBaseTimer(state)
    ...
    if espBaseTimerEnabled then
        ...
        local Plots = Workspace:FindFirstChild('Plots'); local lastValues = {}; local lastChange = {}
                    ...
                    if lowestFloor then
                        ...
                        if main then
                            ...
                            if remainingTime then
                                local currentText = remainingTime.Text or '0'; local key = plot.Name
                                if lastValues[key] ~= currentText then lastValues[key] = currentText; lastChange[key] = now end
                                local numeric = tonumber(currentText); local timeSinceChange = now - (lastChange[key] or 0)
                                if numeric and numeric <= 0 then timerLabel.Text = 'UNLOCKED'; timerLabel.TextColor3 = Color3.fromRGB(0, 255, 0)
                                elseif timeSinceChange > 1 then timerLabel.Text = 'UNLOCKED'; timerLabel.TextColor3 = Color3.fromRGB(0, 255, 0)
                                else timerLabel.Text = currentText; timerLabel.TextColor3 = Color3.fromRGB(255, 50, 50) end
                            else timerLabel.Text = 'UNLOCKED'; timerLabel.TextColor3 = Color3.fromRGB(0, 255, 0) end
                        end
                    end
                end
            end
...
local function startAntiTrap()
    ...
    antiTrapEnabled = true; local blacklist = {"trap", "kill", "lava", "spike", "damage", "void", "web", "slinger"}
    antiTrapLoop1 = task.spawn(function()
        while antiTrapEnabled do for _, v in ipairs(Workspace:GetDescendants()) do if v:IsA("BasePart") then for _, word in ipairs(blacklist) do if v.Name:lower():find(word) then pcall(function() v:Destroy() end); break end end end end; task.wait(1) end
    end)
    antiTrapLoop2 = task.spawn(function()
        while antiTrapEnabled do if player.Character and player.Character:FindFirstChild("HumanoidRootPart") then player.Character.HumanoidRootPart.Anchored = false; player.Character.Humanoid.Sit = false end; task.wait(0.5) end
    end)
    print("✅ Anti Trap: ON")
end

local function stopAntiTrap()
    antiTrapEnabled = false; if antiTrapLoop1 then task.cancel(antiTrapLoop1); antiTrapLoop1 = nil end; if antiTrapLoop2 then task.cancel(antiTrapLoop2); antiTrapLoop2 = nil end; print("❌ Anti Trap: OFF")
end

local function toggleAntiTrap(state)
    if state then startAntiTrap() else stopAntiTrap() end
end

player.CharacterAdded:Connect(function(newCharacter) if antiTrapEnabled then task.wait(1); startAntiTrap(); print("🔄 Reloaded anti-trap after respawn") end end)
...
-- ==================== INSTANT GRAB FUNCTION ====================
local function getPromptPosition(prompt)
    local parent = prompt.Parent; if parent:IsA("BasePart") then return parent.Position end
    ...
    if parent:IsA("Attachment") then return parent.WorldPosition end; return nil
end

local function findNearestPrompt()
    local character = player.Character; if not character then return nil, math.huge end
    local hrp = character:FindFirstChild("HumanoidRootPart"); if not hrp then return nil, math.huge end
    local nearest = nil; local minDist = math.huge; local plots = workspace:FindFirstChild("Plots"); if not plots then return nil, math.huge end
    for _, obj in pairs(plots:GetDescendants()) do
        if obj:IsA("ProximityPrompt") and obj.Enabled and obj.ActionText == "Steal" then
            local pos = getPromptPosition(obj); if pos then local dist = (hrp.Position - pos).Magnitude; if dist <= obj.MaxActivationDistance and dist < minDist then minDist = dist; nearest = obj end end
        end
    end
    return nearest, minDist
end

local function activatePrompt(prompt) fireproximityprompt(prompt, 20, math.huge); prompt:InputHoldBegin(); prompt:InputHoldEnd() end
...
local function startInstantGrab()
    ...
    instantGrabThread = task.spawn(function()
        local currentPrompt = nil; local currentDistance = math.huge; local lastUpdate = 0
        RunService.Heartbeat:Connect(function() local now = tick(); if now - lastUpdate >= 0.05 then currentPrompt, currentDistance = findNearestPrompt(); lastUpdate = now end end)
        while instantGrabEnabled do
            local character = player.Character
            if character then
                local humanoid = character:FindFirstChildOfClass("Humanoid")
                if humanoid and humanoid.WalkSpeed > 25 then if currentPrompt and currentDistance <= currentPrompt.MaxActivationDistance then activatePrompt(currentPrompt); task.wait(1.5) else task.wait(0.1) end else task.wait(0.5) end
...
local function stopInstantGrab()
    ...
    instantGrabEnabled = false; print("❌ Instant Grab: OFF"); if instantGrabThread then task.cancel(instantGrabThread); instantGrabThread = nil end
end
...
local function findPlayerPlot()
    local plots = workspace:FindFirstChild("Plots"); if not plots then warn("❌ Plots folder not found!"); return nil end
    local playerBaseName = player.DisplayName .. "'s Base"
    for _, plot in pairs(plots:GetChildren()) do
        if plot:IsA("Model") or plot:IsA("Folder") then
            local plotSign = plot:FindFirstChild("PlotSign")
            if plotSign and plotSign:FindFirstChild("SurfaceGui") then
                local surfaceGui = plotSign.SurfaceGui; if surfaceGui:FindFirstChild("Frame") and surfaceGui.Frame:FindFirstChild("TextLabel") then local plotSignText = surfaceGui.Frame.TextLabel.Text; if plotSignText == playerBaseName then print("✅ Found player's plot:", plot.Name); return plot end end
            end
        end
    end
    warn("❌ Player's base not found!"); return nil
end

local function findCollectZone(plot)
    if not plot then return nil end; local decorations = plot:FindFirstChild("Decorations"); if not decorations then warn("❌ Decorations not found!"); return nil end
    for _, decoration in pairs(decorations:GetChildren()) do if decoration.Name:lower():find("collect") or decoration:FindFirstChild("CollectZone") then print("✅ Found CollectZone:", decoration.Name); return decoration end; for _, child in pairs(decoration:GetDescendants()) do if child.Name:lower():find("collect") and (child:IsA("Part") or child:IsA("Model")) then print("✅ Found CollectZone in:", decoration.Name); return child end end end
    local children = decorations:GetChildren(); if #children >= 11 then print("✅ Using 11th decoration as CollectZone"); return children[11] end; warn("❌ CollectZone not found!"); return nil
end

local function createPlotLine()
    local Character = player.Character; if not Character then return false end; local RootPart = Character:FindFirstChild("HumanoidRootPart"); if not RootPart then return false end
    local playerPlot = findPlayerPlot(); if not playerPlot then warn("❌ Cannot find your base!"); return false end
    local collectZone = findCollectZone(playerPlot); if not collectZone then warn("❌ Cannot find CollectZone!"); return false end
    local targetPosition
    if collectZone:IsA("Model") and collectZone.PrimaryPart then targetPosition = collectZone.PrimaryPart.Position; elseif collectZone:IsA("BasePart") then targetPosition = collectZone.Position; elseif collectZone:FindFirstChildWhichIsA("BasePart") then targetPosition = collectZone:FindFirstChildWhichIsA("BasePart").Position else warn("❌ Cannot get position from CollectZone!"); return false end
    print("📍 Creating line to CollectZone at:", targetPosition)
    baseTargetPart = Instance.new("Part"); baseTargetPart.Name = "PlotLineTarget"; baseTargetPart.Size = Vector3.new(0.1, 0.1, 0.1); baseTargetPart.Position = targetPosition; baseTargetPart.Anchored = true; baseTargetPart.CanCollide = false; baseTargetPart.Transparency = 1; baseTargetPart.Parent = workspace
    baseBeamPart = Instance.new("Part"); baseBeamPart.Name = "PlotLineBeam"; baseBeamPart.Size = Vector3.new(0.1, 0.1, 0.1); baseBeamPart.Transparency = 1; baseBeamPart.CanCollide = false; baseBeamPart.Parent = workspace
    local att0 = Instance.new("Attachment"); att0.Name = "Att0"; att0.Parent = baseBeamPart; local att1 = Instance.new("Attachment"); att1.Name = "Att1"; att1.Parent = baseTargetPart
    baseBeam = Instance.new("Beam"); baseBeam.Name = "PlotLineBeam"; baseBeam.Attachment0 = att0; baseBeam.Attachment1 = att1; baseBeam.FaceCamera = true; baseBeam.Width0 = 0.3; baseBeam.Width1 = 0.3; baseBeam.Color = ColorSequence.new(Color3.fromRGB(100, 0, 0)); baseBeam.Transparency = NumberSequence.new(0); baseBeam.LightEmission = 0.5; baseBeam.Parent = baseBeamPart
    local pulseTime = 0; local animateConnection; animateConnection = RunService.Heartbeat:Connect(function(dt) if baseBeam and baseBeam.Parent then pulseTime = pulseTime + dt; local pulse = (math.sin(pulseTime * 2) + 1) / 2; local r = 100 + (155 * pulse); baseBeam.Color = ColorSequence.new(Color3.fromRGB(r, 0, 0)) else if animateConnection then animateConnection:Disconnect() end end end)
    baseLineConnection = RunService.Heartbeat:Connect(function() local char = player.Character; if not char or not char.Parent then stopPlotLine(); return end; local root = char:FindFirstChild("HumanoidRootPart"); if root and baseBeamPart and baseBeamPart.Parent then baseBeamPart.CFrame = root.CFrame end end)
    print("✅ Base line created!"); return true
end

local function stopPlotLine()
    if baseLineConnection then baseLineConnection:Disconnect(); baseLineConnection = nil end; if baseBeamPart then baseBeamPart:Destroy(); baseBeamPart = nil end; if baseTargetPart then baseTargetPart:Destroy(); baseTargetPart = nil end; if baseBeam then baseBeam:Destroy(); baseBeam = nil end; print("🛑 Base line removed")
end

local function toggleBaseLine(state)
    baseLineEnabled = state; if baseLineEnabled then pcall(createPlotLine) else pcall(stopPlotLine) end
end

player.CharacterAdded:Connect(function(newChar) task.wait(1); if baseLineEnabled then pcall(stopPlotLine); task.wait(0.5); pcall(createPlotLine) end end)
