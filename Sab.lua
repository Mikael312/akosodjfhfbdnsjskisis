...
local function toggleEspBaseTimer(enabled)
    espBaseTimerEnabled = enabled
    ...
    if espBaseTimerEnabled then
        ...
        espBaseTimerConnection = RunService.Heartbeat:Connect(function()
            pcall(function()
                local plots = workspace:FindFirstChild("Plots")
                if not plots then return end

                for _, plot in pairs(plots:GetChildren()) do
                    -- Look for any GUI that might contain a timer
                    for _, gui in pairs(plot:GetDescendants()) do
                        if gui:IsA("SurfaceGui") or gui:IsA("BillboardGui") then
                            -- Check if it has a label that looks like a timer
                            for _, label in pairs(gui:GetDescendants()) do
                                if label:IsA("TextLabel") and label.Text:match("%d+:%d+") then
                                    gui.AlwaysOnTop = true
                                    gui.MaxDistance = 2000 -- Make it visible from very far
                                    label.TextScaled = true
                                    label.Font = Enum.Font.SourceSansBold
                                    label.TextColor3 = Color3.new(1, 1, 0) -- Yellow text for visibility
...
-- MAIN TAB (4 TOGGLES - Added Xray Base and Invisible V1)
tabContent["Main"] = {}
...
table.insert(tabContent["Main"], createToggleButton("Invisible V1", function(state)
...
-- MISC TAB (4 TOGGLES - Added Grapple Speed, Anti Knockback, and Anti Ragdoll)
tabContent["Misc"] = {}
...
print("🔧 Functions Added: ESP Players (FIXED), Aimbot (Laser Cape), Esp Turret (Sentry), Anti Debuff (Combined Anti Bee & Boogie), Base Line, Platform, Xray Base, Esp Best, Esp Base Timer (FIXED), Grapple Speed, Anti Knockback (NEW), Anti Ragdoll (NEW), Invisible V1 (NEW)")
print("🆕 Added Toggles: Esp Best, Esp Base Timer, Base Line, Platform, Xray Base, Grapple Speed, Anti Knockback, Anti Ragdoll, Invisible V1")
...
print("✅ ADDED: Grapple Speed toggle in Misc tab.")
print("✅ ADDED: Anti Knockback toggle in Misc tab.")
print("✅ ADDED: Anti Ragdoll toggle in Misc tab.")
print("✅ ADDED: Invisible V1 toggle in Main tab.")
print("==========================================")
print("📐 Size: 320x420 (Wider + Taller)")
print("📁 Tabs: Main | Visual | Misc | Discord")
print("📜 Scrolling: YES")
print("🖱️ Draggable: YES")
print("🎮 Style: Arcade theme")
print("✅ Tab switching: FIXED!")
print("==========================================")
