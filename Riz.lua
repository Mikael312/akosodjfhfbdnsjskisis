...
-- OTHER FUNCTIONS
local function enableAutoTp() end
local function disableAutoTp() end
local function enableStealBar() end
local function disableStealBar() end
local function enableInstantClone() end
local function disableInstantClone() end
local function enableTpToBest() end
local function disableTpToBest() end
local function enableRagdollSelf() end
local function disableRagdollSelf() end
...
-- LETAK SINI selepas kamera
    ...
    if animController then
        ...
        local animFolder = ReplicatedStorage.Animations.Animals:FindFirstChild(animalData.modelName)
        if animFolder then
...
local instantCloneBtn = createButton("Instant Clone", 45, "InstantClone", function(ns, set)
    set(ns)
    if ns then enableInstantClone() else disableInstantClone() end
end)

local tpToBestBtn = createButton("Tp to Best", 80, "TpToBest", function(ns, set)
    set(ns)
    if ns then enableTpToBest() else disableTpToBest() end
end)

local ragdollSelfBtn = createButton("Ragdoll Self", 115, "RagdollSelf", function(ns, set)
    set(ns)
    if ns then enableRagdollSelf() else disableRagdollSelf() end
end)
...
local hopServerToggle = createToggle("Hop Server", 185, "HopServer", function(ns, set)
    set(ns)
    hopActive = ns
    if ns then
        task.spawn(function()
...
local settingsToggle = createToggle("Settings", 220, "Settings", function(ns, set)
    set(ns)
...
if featuresContent then
    ...
    createTabToggle(featuresContent, "Auto Tp on Start", "AutoTponStart", function(ns, set)
        set(ns)
        if ns then enableAutoTp() else disableAutoTp() end
    end)
end
