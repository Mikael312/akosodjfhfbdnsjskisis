...
local function getPlotBlockBounds(plot)
    ...
    return {
        ...
        minY = pos.Y - halfSizeY, maxY = pos.Y + halfSizeY,
    }
end
...
local function equipFlyingCarpet()
    ...
    local carpet = player.Backpack:FindFirstChild(Config.CarpetTool or "Flying Carpet")
        or character:FindFirstChild(Config.CarpetTool or "Flying Carpet")
    if carpet then humanoid:EquipTool(carpet) end
end

local function teleportToHighest()
    if #allAnimalsCache == 0 then
        showNotification({message = "No animals found!", color = "Failed", textColor = "White"})
        return
    end
    ...
    if not bestPet then
        showNotification({message = "No target found!", color = "Failed", textColor = "White"})
        return
    end
    ...
    local plot = S.Workspace:FindFirstChild("Plots") and S.Workspace.Plots:FindFirstChild(bestPet.plot)
    if not plot then return end

    local targetPos = nil
    local podiums = plot:FindFirstChild("AnimalPodiums")
    local animalFolder = podiums and podiums:FindFirstChild(bestPet.slot)
    ...
    if animalFolder then
        ...
        if #allParts > 0 then
            ...
            if closest then targetPos = closest.Position end
        end
    end
    ...
    if not targetPos then
        showNotification({message = "Position not found!", color = "Failed", textColor = "White"})
        return
    end
    ...
    if info then
        ...
        finalPos = Vector3.new(targetPos.X, animalY > 10 and 20 or animalY, targetPos.Z)
    ...
    local forwardOffset = targetPos.Y > 10 and 2 or 0.7
