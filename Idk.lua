...
local lastEspBestUpdate = 0

-- ==================== DEBUG HELPER ====================
local function debugError(label, err)
    local info = debug.getinfo(2, "Sl")
    local src = info and info.short_src or "unknown"
    local line = info and info.currentline or -1
    warn(string.format("[ERROR] %s | File: %s | Line: %d | %s", label, src, line, tostring(err)))
end
...
local function formatNumber(num)
    ...
    else return string.format("%.0f/s", num)
    end
        ...
        if not isPlayerPlot(plot) then
            for _, obj in pairs(plot:GetDescendants()) do
                if obj:IsA("Model") and AnimalsModule and AnimalsModule[obj.Name] then
                    safeCall("findHighestBrainrot", function()
...
local function createHighestValueESP(brainrotData)
    ...
    safeCall("createHighestValueESP", function()
        ...
        if not part then
            warn("[ESP Best] part nil untuk model: " .. tostring(model.Name))
            return
        end
        ...
        local bestBadge = Instance.new("Frame", bgFrame)
        ...
        Instance.new("UICorner", bestBadge).CornerRadius = UDim.new(0, 5)
        local bestLabel = Instance.new("TextLabel", bestBadge)
        ...
        local ownerLabel = Instance.new("TextLabel", bgFrame)
        ...
        local distanceLabel = Instance.new("TextLabel", bgFrame)
        ...
        local updateConnection = S.RunService.Heartbeat:Connect(function()
            ...
            if pRoot and part and part.Parent then
                distanceLabel.Text = calculateDistance(pRoot, part) .. "m"
            end
        end)
...
local function enableEspBest()
    ...
    espBestEnabled = true
    safeCall("enableEspBest - initial scan", updateHighestValueESP)
    espBestConnection = S.RunService.Heartbeat:Connect(function()
        if not espBestEnabled then return end
        local now = os.clock()
        if now - lastEspBestUpdate < 3 then return end
        lastEspBestUpdate = now
        safeCall("espBest - updateLoop", updateHighestValueESP)
    end)
end
